...
2. Run `pnpm exec totem lint` — the deterministic enforcement floor (zero LLM). Fix any violations before proceeding; this is the gate the pre-push hook enforces.
3. Run `pnpm exec totem review` — supplementary AI lanes over the diff (advisory sensors, not a merge gate). Address critical findings; known limits are disclosed in the run output (LLM window truncation on large diffs; non-code files skipped). The review of record is the repo's own review discipline, not this pass.
