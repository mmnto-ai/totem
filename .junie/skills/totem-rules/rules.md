...
- **2026-03-06T10:00:40.352Z** — **Pattern:** \b(echo|run*shell_command|exec|sh|bash)\b.*\$TOOL*INPUT
  **Engine:** regex
...
- **Windows requires shell:true for git binary resolution** — **Pattern:** execFileSync\s*\(\s*['"]git['"](?![^)]_shell:\s_(?:true|IS*WIN))
  **Engine:** regex
...
- **Wrap user-controlled fields like PR descriptions** — **Pattern:** (?=._\$\{._(?:description|comment|body|user*?input|userInput).*\})(?!._untrusted[ _-]content)
  **Engine:** regex
...
- **Treat usage and token statistics as optional fields** — **Pattern:** \busage\s*\.\s*(prompt*tokens|completion_tokens|total_tokens)\b
  **Engine:** regex
