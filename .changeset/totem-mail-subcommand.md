...
feat(cli+core): `totem mail` cross-repo outbox poll subcommand (mmnto-ai/totem#1970)
...
New `totem mail` subcommand surfaces unread mail addressed to this repo's agent(s) by scanning sibling-repo outboxes per [ADR-106 § 3](https://github.com/mmnto-ai/totem-strategy/blob/main/adr/adr-106-ephemeral-coordination-layer.md). Replaces ad-hoc per-hook implementations with one cohort-portable surface so consumer repos (`totem`, `liquid-city`, `arhgap11`, `totem-status`, etc.) pick it up via the cohort sync rather than each maintaining its own copy.
