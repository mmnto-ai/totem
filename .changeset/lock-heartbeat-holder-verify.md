...
- **MCP `add_lesson` no longer loses lessons to a long-held lock.** Under a live full re-index it writes the lesson file without contending on the lock; on every other path its acquisition is bounded (~7.5s, `maxRetries: 4`) and a timeout — which proves a live long holder, since a dead or stale one would have been reclaimed inside those retries — falls back to the same lockless write with the convenience sync deferred. Previously an unstealable long hold (paced full OR paced incremental) meant a deterministic ~4-minute block ending in a lost lesson.
