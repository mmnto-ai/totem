...
`evaluationCount` has crossed a configurable window while
...
`TotemConfig.doctor.staleRuleWindow` (default 10) and
`TotemConfig.doctor.minRunsToEvaluate` (default 3) gate the check; a
superRefine rejects configs where the window is smaller than the floor.
