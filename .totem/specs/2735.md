...
| The refusal TEXT contradicts the count line it prints beside                                  | runtime (disclosure)                  | a free-text run on an index holding lesson rows but no matching spec, session or code rows WOULD print `Found: … N lessons` and then refuse with `nothing in the index grounds this run` — two true lines that read as a contradiction, because the floor no longer judges the partition the count line reports. The arm is exercised by tests, not by any recorded run | `formatGroundingRefusal` takes the delivered-lesson count: with lessons delivered it says `0 grounding hits (specs, sessions, code)` and names them — `N lessons were retrieved, but lessons do not ground a run (mmnto-ai/totem#2727 rules whether they may)`. With none delivered the message is byte-identical to the pre-slice text; both arms are pinned by exact-string tests, and a command-level test pins the count reaching the formatter |
...
- **Live falsifier (recorded in the PR, not a unit test):** the headline is the SAME command before and after — `totem spec 2735 --stdout` was served `0 lessons` beside 5 specs and 3 code chunks at design time and is served `10 lessons` (the `MAX_LESSONS` cap) beside the same 5 and 3 after the fix. Then the replay: `.totem/fixtures/spec-runs-2026-09-02/scripts/r1-retrieve.mjs --resident D:/Dev/totem --out <scratch>` through this worktree's built dist delivers ≥ 1 lesson on 56 of 56 queries, where R1 measured 0 of 55. The denominators differ because the harness derives its corpus from the resident's LIVE `.totem/artifacts/runs/`, which gained one spec-caller artifact after the fixture's export; the committed fixture's denominator stays 55. Run artifacts are gitignored, so the replay output is PR-body evidence, not a committed file.
...
- **Question 4:** Re-run the 55-query R1 replay after the fix and publish the count in the PR body?
  ...
  - **Recommendation:** yes.
- **The F3 fork** (raised by the fold-1 falsification leg, after the four questions above were answered): does delivering lessons loosen the mmnto-ai/totem#2700 grounding floor?
  - Ruled 2026-09-03 after the fold-1 falsification leg: the floor holds its three exercised partitions (leg-2 evidence: mmnto-ai/totem#2700's scope and acceptance text never name a partition; its recorded run measured 0 lessons; “all four” was implementation prose). Stated honestly: mmnto-ai/totem#2700's refusal key is written over “0 items” — “retrieval returns 0 items”, “0 items refuses” — and under this ruling a run may retrieve lesson items and still report 0 grounding hits, which fold 3's refusal text discloses in the message itself. The ruling stands on the behaviour that record actually exercised, and is reversible on the operator's word; the mmnto-ai/totem#2727 floor arm rules lesson grounding.
