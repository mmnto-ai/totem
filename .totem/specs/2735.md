...
| The refusal TEXT contradicts the count line it prints beside                                  | runtime (disclosure)                  | a free-text run on a lesson-holding index with 0 spec/session/code hits printed `Found: … 10 lessons` and then `nothing in the index grounds this run` — two true lines that read as a contradiction, because the floor no longer judges the partition the count line reports                             | `formatGroundingRefusal` takes the delivered-lesson count: with lessons delivered it says `0 grounding hits (specs, sessions, code)` and names them — `N lesson(s) were retrieved, but lessons do not ground a run (mmnto-ai/totem#2727 rules whether they may)`. With none delivered the text is byte-unchanged; both arms are pinned by tests                      |
