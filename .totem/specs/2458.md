...
> **Amendment (review round on PR #2538, 2026-07-31; revised same round).** `SearchIdentity` as drawn is not collision-safe across federation — but a _store_-based namespace would over-correct and leave the filed bug unfixed. The contract, superseding the interface above where they conflict:
>
> ```typescript
> interface SearchIdentity {
>   homeNamespace: string; // the document's HOME repo, derived from the normalized path's repo anchor — NOT the store that returned the hit
>   normalizedPath: string; // repo-relative within homeNamespace
>   sectionIdentifier: string;
> }
> ```
>
> Two hits are the same document iff all three fields match. The same document arriving via two stores or two path spellings — the filed duplication class ("one labeled with the source tag, one unlabeled") — therefore still merges and accumulates RRF score, while same-relative-path documents from different homes never do. Store provenance/tags are merge metadata, never part of the grouping key. If the home anchor cannot be derived for a hit, keep it separate: a surviving duplicate is recoverable downstream, a false merge silently loses a result. Fuzzy section matching must keep ambiguous matches separate. Build-time tests: identical relative paths across different homes stay distinct; the same document via two stores/path spellings merges with accumulated score; underivable-home hits pass through unmerged.
