...
- Expected option sets come from the row — the structured `expectedOptionSets` when present (a scalar where a list belongs is refused, never normalized to one option), else `expectedValueOrDerivation`'s prose; a row yielding no clause renders `unknown`, never a hardcoded pass. A reworded prose clause the grammar still reads renders drift, not `unknown` — the structured field is the cure, the prose only the pinned interim. A test parses the PINNED doctrine row (the installed manifest) and asserts the two sets, so a doctrine reword fails the suite rather than darkening the sensor silently.
- The vocabulary row emits one line for the current repo's bound project and one `skip` per sibling; the label row emits one line per roster repo, and each label message names its repo slug (the `--json` readout carries the row `id` and the message, not the per-repo display name, so the message is what keeps a multi-repo roster's rows distinguishable).
