...
- **core `ParityContract`** (added after the design gate, on strategy's 2026-09-05 ruling): gains `expectedOptionSets?: Record<string, string[]>`, mapped from an optional `expected-option-sets` row field admitted at the raw boundary as `z.unknown()` (the manifest-wide-outage guard the other promoted fields use) and narrowed per row (every key non-empty, every value a non-empty list of non-empty strings, else absent on that row). The vocabulary detector prefers it when present and falls back to the prose grammar when absent; the verdict line names which canon it read (`canon: expected-option-sets` / `canon: expected-value-or-derivation text`). Strategy adds the field to the `gh-project-vocabulary` row and pins the prose grammar in its `source-note` in the 0.1.44 batch.
...
| an expected field absent from the project                              | runtime         | `warn` naming the field (its set is empty, not the canon — the twin's reading; amended from `unknown`, see the open question below) | operator adds the field      |
...
- Color compares case-insensitively without `#` (normalized on both the canon and the live side; the twin strips `#` on the live side only, identical while the script writes bare hex); description compares exactly, untrimmed, as the twin does; a `null` description against canonical text is a redefinition. Option NAMES: the canon's members are trimmed (authoring whitespace — the prose grammar cannot avoid it, a quoted YAML member may carry it), the live side's are compared raw, so a padded board option is a real difference the board shows.
...
- Expected option sets come from the row — the structured `expectedOptionSets` when present (a scalar where a list belongs is refused, never normalized to one option), else `expectedValueOrDerivation`'s prose; a row yielding no clause renders `unknown`, never a hardcoded pass. A reworded prose clause the grammar still reads renders drift, not `unknown` — the structured field is the cure, the prose only the pinned interim. A test parses the PINNED doctrine row (the installed manifest) and asserts the two sets from the PROSE always, and that a structured field, when present, agrees with the prose — so in an AUTHENTICATED checkout a doctrine reword fails the suite rather than darkening the sensor silently. Disclosed: CI cannot install the optional `@mmnto/strategy-doctrine` pack (no registry read auth on the runner, mmnto-ai/totem#2289), so that case is skipped there (vitest reports the skip in its counts; the case's title names the artifact whose absence skips it); the guard is live on every cohort seat's checkout, not on the merge gate, until mmnto-ai/totem#2289 lands. Also disclosed: the same test's structured-field agreement branch is inert at the 0.1.42 pin (the row carries no structured field) and arms when doctrine 0.1.44 lands the field; it compares each field's members as sorted lists, so a duplicated member is caught, order ignored.
- The vocabulary row emits one line for the current repo's bound project and one `skip` per sibling; the label row emits one line per roster repo. Every label message (verdict and cannot-verify alike) and every vocabulary skip names its repo slug; a bound vocabulary line names its project (the `--json` readout carries the row `id` and the message, not the per-repo display name, so the message is what keeps a multi-repo roster's rows distinguishable).
- A GraphQL body carrying ANY `errors` entry is cannot-verify (auth / not-found / error), never `ok` — partial data beside errors is not a field list to verdict.
...
- **Question:** expected option sets derived from the row's text, or hardcoded in the detector?
  ...
  **Resolved (2026-09-05, strategy-claude by mail):** a structured `expected-option-sets` field is the field's contract and the prose grammar the pinned interim; both land on strategy's side in 0.1.44. This slice ships the detector preferring the structured field with the prose parse as the fallback, and the loader's optional-field admission (see the `ParityContract` delta above).
- **Question:** a project missing the `Status` or `Priority` field entirely: `unknown` (the issue text) or `warn`?
  ...
  **Resolved (2026-09-05, at build time, recorded on mmnto-ai/totem#2791):** `warn`. The strategy-local twin reads a missing governed field as a fault ("a fault is a governed field that is absent, or whose option SET differs"), the row's detection-method compares each governed set for equality and an absent field's set is empty, and the done-when requires the two readers to agree on every repo. `unknown` stays for what is genuinely not comparable: the project null or inaccessible, the fields overflowing one page, the payload unshaped, or the row yielding no option sets.
