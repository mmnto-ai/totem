...
The Gemini `SessionStart.js` hook contains CommonJS code but uses a `.js` extension. In ESM consumer repositories (`"type": "module"`), Node.js parses it as ESM and throws an error, causing the Gemini CLI to silently degrade the crash to a warning and skip the session-start briefing (premise falsified in review — see Amendment below). This issue must be fixed by migrating `SessionStart.js` to `SessionStart.cjs`, updating the installation/eject manifests, and introducing a legacy migration path identical to the one implemented for `BeforeTool.cjs` in PR #2485.
...
### Amendment (falsification review, 2026-08-04)

The Problem Statement's premise "Gemini CLI degrades the crash to a warning" was falsified during review: per mmnto-ai/totem#2558, no `.gemini/settings.json` SessionStart registration is ever emitted and Gemini CLI has no filename-convention discovery, so Gemini CLI never executes this hook at all. The fail-open bites the host/plain-node execution paths — which is how totem-status reproduced it live. The `.cjs` fix is unchanged; arming the hook in Gemini CLI remains #2558's scope. Also generated stale: the "Files to Examine" paths — `packages/cli/templates/` does not exist (templates are inline constants in `init-templates.ts`) and there is no `packages/cli/test/` tree — the implementation built to the real file map while honoring the three TEST DIRECTIVE names verbatim.
