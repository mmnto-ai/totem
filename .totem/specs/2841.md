...
**Where the generated spec above disagrees with shipped contracts, and which wins:** (1) there is no `packages/cli/src/commands/review-reply.ts` — review-reply is a managed SKILL (`REVIEW_REPLY_SKILL_CONTENT`), not a command family; the verb is a new command module `resolve-threads.ts` wired in `packages/cli/src/index.ts`. (2) `BOT_REVIEWER_IDENTITIES` lives in `packages/core/src/bot-identity.ts` (mmnto-ai/totem#2800), not a constants file. (3) The spec's evidence rule (an in-thread human reply only) would make every GCA thread unresolvable, because bot-protocols forbids replying to GCA in-thread; the evidence rule below admits a later human PR-level comment. (4) The spec's `--thread` / `--finding` selection flags are replaced by the selection rule in the open questions (resolve every evidenced bot thread by default; ids optional). (5) `comments.nodes[0]` as the root holds, but the read must be paginated and its failure is a hard error, not a partial list. (6) The spec's edge case 1 says only that a null author "must not be classified as a valid bot identity"; the build makes that a POSITIVE rule with a consequence the spec does not state — a deleted account's in-thread reply still COUNTS AS EVIDENCE, because the reply was a human reply when it was written and deleting the account does not retract it. (The same null author at a thread's ROOT means the thread is not bot-rooted, so it is never a candidate.) Recorded here because it is a judgement, not a derivation: the opposite reading — treat an unattributable author as no evidence — is defensible and was not taken. Added during the mmnto-ai/totem#2841 fold round, alongside widening the bot test to every GitHub App (`__typename: Bot` / `[bot]` suffix / core's list), which the spec's "contained in `BOT_REVIEWER_IDENTITIES`" phrasing would have left failing open.
...
<!-- totem-context: the preflight's approval gate for mmnto-ai/totem#2841; the design is the "Implementation Design" section of .totem/specs/2841.md on branch spec/2841-preflight; every open question below is now ruled and the build starts from this record -->

| #   | Question                  | Ruled                                                                                                                                                                                                  |
| --- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| R1  | Verb shape                | A new `totem resolve-threads <pr>` verb, dry-run by default, `--apply` to mutate. `triage-pr` stays the read inbox.                                                                                    |
| R2  | Evidence for a decline    | An in-thread human reply OR a human PR-level comment created after the thread's root — either suffices. One rule for every bot; a thread with neither is listed as needing a reply and never resolved. |
| R3  | Where the skill step sits | Inside the review-reply skill's `done` step, after the round-disposition comment posts, as the last operator-gated action; the disposition comment is the evidence the verb reads.                     |
| R4  | Selection                 | Resolve every evidenced bot thread by default; `--ids` (REST root comment ids, printed beside each row) narrows it; an unmatched id aborts before any mutation. No inbox-number selection.             |

Standing from the design: the bot-identity list is core's `BOT_REVIEWER_IDENTITIES` (mmnto-ai/totem#2800), so the build lands after that PR merges; the read is paginated and a failed or incomplete read is a hard error, never a partial plan; the verb never posts a comment or a review; the three distributed review-reply surfaces stay byte-locked to the constant with string locks on the new step.

Build sequence: worktree off main once mmnto-ai/totem#2800 is on main, the spec branch folded into the build PR; the build is legs-owed (the managed skill constant, `.claude/skills/**`, `.agents/skills/**`, a changeset) and takes one falsification leg before it is presented.
