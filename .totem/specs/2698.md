...
- **Predicate inputs** — per invocation: `getDiffForReview({ branch: true })` (the push-gate scope lint uses) gives `changedFiles`; globs come from the config loaded at run time. A `DiffForReviewEmpty` result is not owed (nothing to read), printed as such.
