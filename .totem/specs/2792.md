...
| a form's `labels:` entry is not a canon name (a space-less `type:bug`)       | permanent | GitHub silently creates a squatter label                                                      | the forms-coupling test fails the PR                                                                             |
...
| `-WhatIf` shadow leaking into a real run                                     | permanent | labels never written, output looks green                                                      | the dry-run test asserts a stubbed `gh` receives ≥ 24 label calls when `-WhatIf` is absent and zero when present |
...
Remaining open questions for the operator: 2 (`-WhatIf`), 3 (auth preflight), 4 (test placement), 6 (security form) — each with its recommendation above.
