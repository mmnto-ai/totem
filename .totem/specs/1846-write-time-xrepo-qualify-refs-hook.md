...
Extract the regex `/(^|[^a-zA-Z0-9/_-])#(\d{2,4})([^0-9]|$)/g` into a shared export in the core package (e.g., `BARE_REF_REGEX_STRING`) so both the `xrepo-qualify-refs` linter and the `init` templates can use it as a string to inject into the hook templates.
