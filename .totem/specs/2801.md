# mmnto-ai/totem#2801 — signon derives the seat first: the preflight record

<!-- totem-context: three layers, each superseding the one above it where they differ. (1) The Phase 1 sections — `Problem Statement` through the generated task list — are `totem spec` RETRIEVAL output: lessons the corpus returned and a plan the model generated, one input to the design and never the contract. Its `.handoff/` scan, its "exit 2 or 4", its "print the seat name", its `expected-action` BLIND parse and its SessionStart suppression knob are exactly what the design and the rulings below decline: no substrate scan and no wrong-single-identity detector (the charter's standing — the env is an operator declaration, and the CLI validates hosted-seat MEMBERSHIP only), exit 2 only, the canonical `seat=<id> source=env` line, the literal subject prefix `BLIND round: ` (R3), and no hook growth and no suppression knob (R1 — a separate issue). (2) `Implementation Design` is the Phase 3 contract the build was greenlit on. (3) The `Phase 4 rulings` table binds it. The AS-BUILT record is the changeset `.changeset/signon-derive-seat-2801.md`; `As-built deltas` at the end of this file names where the build went past the design. -->
...
<!-- totem-context: the preflight's approval gate for mmnto-ai/totem#2801; the design is the "Implementation Design" section of .totem/specs/2801.md on branch spec/2800-2801-preflight; every open question below is now ruled and the build starts from this record -->

| #   | Question                                                                                                  | Ruled                                                                                                                                                                                                                                                                                                                                   |
| --- | --------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R1  | The managed SessionStart hook: grow it to inject journal and mail, or make the skill name what it injects | The skill names exactly the two blocks the managed hook injects (describe, orient --session) and derives the seat's journal and its seat-anchored mail by hand on every seat, matching the 2026-09-07T03:40Z datum. Growing the managed hook is a separate issue and needs its suppression knob designed before any injection is added. |
| R2  | Verb home                                                                                                 | totem mail --derive-seat, as mmnto-ai/totem-strategy#956 asked, implemented as one call into the resolver totem seat already uses; a whoami alias later is one line.                                                                                                                                                                    |
| R3  | The BLIND-kit marker literal                                                                              | Resolved by strategy-claude 2026-09-07T18:40Z: the dispatch subject's literal prefix "BLIND round: " with the deadline clause; the signon text quotes it verbatim. Sensing it as a frontmatter key is a separate mechanism ask.                                                                                                         |
| R4  | AGENTS.md ordering                                                                                        | One sentence in totem's Session Start Protocol: on a hook-less seat, signon's seat-and-assignment step precedes every knowledge search. The cohort default stays strategy's lane.                                                                                                                                                       |

Standing from the charter and its two comments: step 0 reads the inherited TOTEM_SELF_AGENT first, an inline form may only restate it, the stop-and-ask is for the empty case only; no wrong-single-identity detector is attempted (env is an operator declaration); no substrate scan; the three distributed signon surfaces stay byte-locked to the constant with new string locks on the added clauses.

Build sequence for the next session: the preflight branch is the contract; the build touches the distributed skill constant in init-templates.ts and so is legs-owed, taking one falsification leg before it is presented; the mail command change and the skill change may ship as one PR.

## As-built deltas — where the build went past the design above (recorded after PR mmnto-ai/totem#2843's first review round)

<!-- totem-context: the changeset `.changeset/signon-derive-seat-2801.md` is the as-built record; this list names only what a reader of the design's task list would otherwise miss -->

- **R4 shipped:** totem's `AGENTS.md` Session Start Protocol carries the ordering sentence (signon's seat-and-assignment step precedes every knowledge search on a hook-less seat); the design's file list and task list above predate the ruling and omit the file.
- **The source of the three signon surfaces is `SIGNON_SKILL_CONTENT`** in `packages/cli/src/commands/init-templates.ts`; `.claude/skills/signon/SKILL.md` and `.agents/skills/signon/SKILL.md` are RENDERED from it and byte-locked to it by test — the design's "update the skill markdown" step is executed by editing the constant and re-rendering, never the rendered files.
- **The cohort seat map is keyed on the git origin's repository** (with the basename as the origin-less fallback), and the resolver scrubs `GIT_DIR`-class variables from the git read — the design assumed the basename key.
- **Identity validation is hosted-seat membership, nothing more:** the env is an operator declaration; the Phase 1 lessons' "validate against the session's actual seat" has no verifiable source and was not built.
