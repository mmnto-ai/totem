...
- **Step 2: Kit-First Enforcement**
  - Statically define the order: classifier sessions read *only* the kit and the artifact named by the kit.
...
- **Question:** What literal marks a BLIND-kit dispatch so step 1 can end signon on it? **Options:** a subject prefix; an `expected-action` value; a frontmatter field. **Recommendation:** strategy-claude names it from `kit-set2-template.md` (their lane); the skill quotes it verbatim; until named, the text says "a directed dispatch that declares a BLIND round".
