...
8. **Window-awareness (CRITICAL):** `resolveSelectionRule` now applies `sel.window` — `all` = every qualifying PR; `bounded { n }` = the **N most-recent qualifying PRs** (the resolver receives metas in git-log newest-first order, so it takes the first N qualifying after predicate + revert filtering). Without this a bounded lock would re-derive the full history and false-fail. **NB — flag to strategy-claude:** the bounded _count_ semantics ("N most-recent qualifying") is my defensible default; the 0432Z/0506Z dispatches pinned the membership predicate, not the window-count rule. Non-blocking — the certifying corpus is `window: all` per their empirics (≈310 merged / ~117 code-touching); bounded is not the certifying path.
   ...
   **RESOLVED (strategy-claude 0810Z, in-reply-to 0758Z):** the bounded count semantics is **CONFIRMED** — `bounded { n }` = the N most-recent _qualifying_ PRs (the **predicate** decides _qualifying_; the **window** decides _how many_, by recency over the already-filtered set — not "last N merged, then filter"). With **one pinned invariant:** "most-recent" must be a **topology-deterministic** ordering (`--topo-order` / ancestry walk), NOT default `git log` order. Default `git log` is _commit-date_ ordering, and commit dates are exactly the rebase-rewritable wall-clock §6 bans for the ancestry boundary — a date-based recency sort would make a bounded corpus non-deterministic (a rebase that rewrites dates could change _which_ N PRs land), breaking the same determinism §6 protects for membership. The invariant is identical, just applied to the window's selection instead of the boundary. **Found + fixed in #2197:** `enumeratePrMetas` was using default `git log` order → now passes `--topo-order` (a contract test locks the flag at the enumeration seam). **Under-N:** if fewer than N qualify, return all of them, never an error — `qualifying.slice(0, n)` does exactly this. **Home for the semantics:** spec-level (this section), NOT a §6 amendment — `bounded` is non-certifying so it never gates a certifying run (strategy-claude's lean, accepted; the offer to bind it at §6 contract altitude was declined as unnecessary).
