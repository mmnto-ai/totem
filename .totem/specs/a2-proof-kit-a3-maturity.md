...
| `docs/data/lint-receipt.json`                                                                                 | `{baseSha, headSha, filesChanged, rules, errors, warnings, elapsedMs, llmCalls: 0, apiKeysStripped: true, platform, node, cliVersion, generatedAt}` | `tools/gen-lint-receipt.mjs` (runs workspace `totem lint` over a pinned REAL merged-PR range of this repo) | LINT_RECEIPT transform; CI recompute job                               | `llmCalls` must be literal 0; counts must match CI recompute (timing is env-labeled, not gated)                                                                                                                                                                                 |
