...
- **Hallucinated file.** `packages/core/src/parity-detect.ts` does not exist and no "core TS agent resolver" mirrors the hook's detection block. The only copies of the block are the renderer (`buildAgentDetectionBlock` in `packages/cli/src/commands/install-hooks.ts`), rendered into both hook templates, and this repository's own `tools/pre-commit` and `tools/pre-push`, which `tools-hook-parity.test.ts` byte-locks to the renderer's output; those two are regenerated from the built renderer, not hand-edited. The "sync anchors" invariant the spec cites therefore has no second copy to anchor.
- **Not followed, the doctor sensor.** A `totem doctor` row attesting that the arm fires under the current shell (the issue's optional half of option 1) is out of this slice; recorded on the issue as a follow-up, not built here. The `HookDiagnostics` interface the spec proposes is not adopted.
- **Not followed, the directive's test name and file.** The lock lives in `install-hooks.test.ts` beside the existing agent-detection cases, with a descriptive title in the file's own convention: a control run with every marker scrubbed passes, and one run each under `CLAUDECODE=1` and `CLAUDE_CODE_ENTRYPOINT=cli` alone is BLOCKED for missing spec evidence. RED on the previous block.
- **Beyond the spec, followed from the issue thread.** `CLAUDE_CODE_ENTRYPOINT` is added as the second marker (lc-claude's 2026-09-13 measurement: a stable value `cli` beside `CLAUDECODE=1`); the two never-observed names stay for explicit exports. The managed CLAUDE.md sentence names the variables and the opt-in for other seats (Tenet 19), and `REFLEX_VERSION` moves to 16 so `totem init` refreshes the block. The changeset is `minor` for the fixed group and discloses the behaviour change for Claude Code seats at the next hook re-render.
- **Edge case answered.** POSIX `[ -n "$VAR" ]` on an unset variable is the empty string under `set -u` only if the shell is that strict; the existing block already relies on the same form for three names and the hook does not set `-u`. No change.
