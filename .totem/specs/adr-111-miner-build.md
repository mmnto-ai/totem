...
- **§5 `excludedPrs` scope — pinned to model A** (codex-confirmed): `corpus` is the cover base that **includes** the PRs later assigned to `excludedPrs` (reverts retained in the base, enumerated explicitly), so `train ⊎ heldOut ⊎ excluded == corpus`. The fail-loud `excludedPrs ⊆ corpus` guard enforces it. Final word pending strategy-claude's §5 reply (`0422Z`).
