...
- **`DraftClassifier`** (injected port, core-defined / CLI-implemented — the `Stage4VerifierDeps`/`DraftExtractor` DI pattern): `classify(draft: DraftCandidate) → Promise<ClassifierDisposition>`. **Error contract (mirrors `DraftExtractor`'s `[]`-on-failure):** the adapter catches its own LLM/network errors and returns the **safe default `behavioral`** (RAG-only — the conservative low-privilege route, Tenet-9; Stage-4-backstopped). It MUST NOT throw for a per-candidate failure; a contract-violating throw propagates loudly (core adds no swallowing catch — Tenet 4). The classifier is **seed-blind** (§7 / FM(f)) — the port is handed only a `DraftCandidate` (provenance + dslSource), no seed channel by construction.
- **`ClassifyStageDeps`** = `{ classifier: DraftClassifier; seedClassesProvided: boolean }` — the `seedClassesProvided` is the **run-level** fact threaded from `ExtractStageResult.seedBlindness` (the classify stage adds no seed channel, so it never flips it; single fact spanning extract+classify per §7).
