...
| --------------------------------------------------------------------------------------------------------------- | -------- | -------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| 2.x runtime wire-format quirk not visible from `.d.ts` (e.g., response field renamed at JSON layer)             | runtime  | First post-merge `totem sync` / `totem compile` (Gemini route) fails at API call | Hot-fix PR — patch the call site; risk class anchored by `feedback_test_invariant_check_before_fix` |
...
| Test mocks at `vi.mock('@google/genai', ...)` shadow a 2.x type-only change                                     | runtime  | Production-only failure that tests don't catch                                   | Acknowledged risk; ratified skip-smoke per strategy-claude `2024Z` reply                            |
