# mmnto-ai/totem#2800 — merge-ready gate: the preflight record

<!-- totem-context: three layers, each superseding the one above it where they differ. (1) The Phase 1 sections — `Problem Statement` through the generated task list — are `totem spec` RETRIEVAL output: lessons the corpus returned and a plan the model generated, one input to the design and never the contract; their `GateVerdict.decision`/`metadata` schema, their REST fallback and their wrapper-level pilot handling are exactly what the design below declines. (2) `Implementation Design` is the Phase 3 contract the build was greenlit on. (3) The `Phase 4 rulings` table binds it. The AS-BUILT record is the changeset `.changeset/merge-ready-gate-2800.md`; where the design and the build differ, `As-built deltas` at the end of this file names each delta. -->
...
| a predicate fails                                                      | runtime   | the ENGINE answers `deny` at BOTH tiers, naming the FIRST failing predicate and its evidence; the host exit is the WRAPPER's map — strict 2, pilot 0 with the deny line printed (a pilot exit 0 is a printed deny, never a pass) | fix the PR                |
...
## As-built deltas — where the build departed from the design above (recorded after PR mmnto-ai/totem#2844's first review round)

<!-- totem-context: the changeset `.changeset/merge-ready-gate-2800.md` is the as-built record; this list names only the points a reader of the design would otherwise get wrong -->

- **Identities: four, not three, and richer than `{ tool, loginPattern }`.** `BOT_REVIEWER_IDENTITIES` carries `coderabbit`, `gca`, `greptile` and `ghcq`, each `{ tool, loginPattern, exactLogins, actorId, actorLogins }`: `loginPattern` is the loose REST-shaped triage match, `exactLogins` the CLOSED list the GraphQL surface is matched against (where `author.login` carries no `[bot]` suffix), `actorId`/`actorLogins` the Layer-B actor resolution. The data-model line above is the design sketch; the module is the contract.
- **Verdict shape:** the ADR-109 `{ disposition, reason, provenance }` as the scope paragraph says — the Phase 1 sections' `decision`/`metadata` schema was never built.
- **The wrapper judges EVERY `gh pr merge` at command position** in a compound command (one engine spawn per merge, one shared 30-second budget), and its position anchor skips the shell's command-position words (`if`, `elif`, `while`, `until`, `do`, `then`, `else`, `time`, `coproc`, `!`, `exec`, `command`, `eval`) and `NAME=value` assignment prefixes. Remaining disclosed misses: a wrapper program that takes operands first (`sudo`, `timeout 30`, `npx`, `env`), a skipped word with a flag (`command -p`, `exec -a`, `time -p`, `time --`), a merge handed over as one quoted word, a backtick substitution, a leading redirection, and PowerShell-specific quoting; the contrived false fires (an array assignment spelling a merge; a `case` pattern `gh pr merge)`; a function definition whose body is a merge) are disclosed in the template.
- **The `reviews` and `reviewThreads` connections are read as strictly as the check rollup:** missing, without `nodes` or `pageInfo`, or carrying an unreadable node — a failed read, named; never an empty complete list.
- **Predicate order ahead of the unreadable-input arm:** a bot HIGH inline with a null `comment.commit` is unevaluable only once predicates 1–3 and every placeable predicate-4 inline have passed; it still preempts predicate 5.
