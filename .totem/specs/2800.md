...
- **The wrapper judges EVERY `gh pr merge` at command position** in a compound command (one engine spawn per merge), and its position anchor skips the shell's command-position words (`if`, `elif`, `while`, `until`, `do`, `then`, `else`, `!`, `exec`, `command`) and `NAME=value` assignment prefixes. Remaining disclosed misses: a wrapper program that takes operands first (`sudo`, `timeout 30`, `npx`, `env`) and PowerShell-specific quoting.
