...
- FTS-only hits are never withheld by a floor, and one of them from a grounding partition (specs, sessions, code) saves a `totem spec` run; a keyword-only lesson does not (lessons never ground a run).
...
- Semver for the removed constant: **major**, ruled at the bot round on 2026-09-04 — the root barrel's own header promises zero removals until a major (`packages/core/src/index.ts`, mmnto-ai/totem#2336), and a deleted public export is breaking by the semver definition (the mmnto-ai/totem#2691 "honest semver" clause); the fixed changeset group takes every `@mmnto/*` package to 2.0.0 together. The build had called it minor on the five-repo import measurement alone. The three packs' `engines['@mmnto/totem']` ranges widen from `^1.26.0` to `>=1.26.0 <3.0.0` in the same PR (ADR-097 § 11: the field is immune to the fixed-group bump, and the boot cross-check would otherwise refuse a 2.0.0 engine for every consumer with a pack installed — the re-armed leg's F1), and each pack's structure test now runs that boot predicate for real.
