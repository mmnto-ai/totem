...
The default `searchRelevanceFloor` of `0.25` is mathematically unreachable when using unit-normalized embeddings (e.g., Gemini, OpenAI) with L2 distance. Because L2 distance is bounded within $[0, 2]$, the calculated relevance `1 / (1 + distance)` is bounded below by `0.333`, rendering the exclusion mechanism inert even for absolute nonsense queries (which score `~0.548`).
...
- A configured floor or a per-call `min_relevance` restores today's behavior byte-for-byte in messages.
