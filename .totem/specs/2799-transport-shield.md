> **Superseded above the line.** Everything from here to `## Implementation Design` is the generated pre-design spec (`totem spec`, gemini-3.5-flash, 2026-09-06), kept for the record; where it differs from the design section — the zod payload schema, a `defaultMatcher` registry shape, prepare-time bundling, a doctor `gate-config` task — the design section governs and the code follows the design (falsification fold F12).
