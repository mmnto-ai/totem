...
## Falsification-leg round 1 — dispositions

Leg deposited 3 MAJOR / 6 MINOR / 3 NIT (two backed by executed mutant harnesses). Owner-verified against primaries; dispositions:

**Accepted and fixed:**

- **MAJOR-1** (E2E cell could not distinguish which probe fired — both flushBuffer probes deletable with the cell green, proven by an executed mutant): checkpoint-contents assertions added (`completedFiles` length + `not.toContain(unflushedFile())`) plus a non-vacuous recovery assertion (`finisher.embedCalls.length > 0`; `store.count()` alone passes on a zero-work resume).
- **MAJOR-2** (unstealable lock turns MCP `add_lesson` into a ~255s block then a LOST lesson during exactly the paced runs this fix creates — the #2569 deferral check sat after the acquire): live-epoch check hoisted; the lesson writes locklessly under a live epoch (content-hash filename; the epoch holder only reads the dir), deferral predicate re-derived at decision time. Cell extended: `acquireLock` must not be called on the live path. Disclosed in the changeset.
- **MAJOR-3** (`beginFullSync`'s marker overwrite + `store.reset()` unprobed — the exact mutation the issue names; reachable via the wx-window race after a manual lock deletion): `assertLockHeld` added at `beginFullSync` entry. No dedicated induced cell (theft-before-first-flush is not deterministically inducible without deeper seams); disclosed as residual for the bot round.
- **MINOR-1** (release deleted a foreign present-but-unreadable lock — executed harness): release now mirrors the beat's unreadable-guard (latch + leave); new cell.
- **MINOR-2/3/4** (assertion-strength gaps: per-field deletion comparison, ghost-beat timer teardown, legacy blocked-while-fresh half): cells strengthened as proposed.
- **MINOR-6** (the SYNC_FAILED hint's "delete it manually" now aborts a live sync if followed): hint reworded to require a non-advancing timestamp first.
- **NIT-1** (patch vs new public API): changeset bumped to minor with the exports enumerated (#2569 precedent).

**Declined, falsifiable:**

- **MINOR-5** (theft latched after the final probe → warn-only exit 0): the remaining unguarded writes (`writeIndexMeta`, `index-manifest.json`) are regenerable caches the thief's own run rewrites; converting the release-time warn to a throw would put a throw path into every short holder's release (registry, add-lesson). Reopens if a concrete harm from those two writes under theft is shown.
- **NIT-2** (wx-reclaim's EEXIST arm untested): structurally unreachable single-process (the beat is synchronous); the arm is three lines in the conservative direction. Reopens with a multi-process harness.
- **NIT-3** (`.hb` temp leak on crash between write and rename): bounded to one file per crashed PID under `.totem/`; a sweep belongs in `doctor`, not this diff.

**Attacked and held** (leg-verified, no change): beats-fire-during-awaits (pacing/backoff are timer awaits at the primaries); CI timing margins structural (shared event loop); lesson-5b1929d1 ordering preserved; consumer/type compat (mcp tsc clean, sub-second holds never reach a beat); no two-holders interleaving converges to double-ownership; Windows rename-over-open fine Node-to-Node.

## Falsification-leg round 2 (scoped to the fold) — dispositions

Round 2 verified all round-1 dispositions discharged (MAJOR-1 mutant-proven dead; ADR-115 bar declared met) and deposited 1 MAJOR / 4 MINOR / 3 NIT. Dispositions:

**Accepted and fixed:**

- **MAJOR-R1** (`hasFullSyncCheckpoint` is a proxy — a long paced INCREMENTAL hold writes no marker, and the fresh-full prelude holds the lock pre-marker; the ~255s-block-then-lost-lesson harm survived on those paths): `AcquireLockOptions.maxRetries` added; `add_lesson` bounds its acquisition (`maxRetries: 4`, ~7.5s) and a `SYNC_FAILED` timeout falls back to the lockless write + sync deferral. The timeout PROVES a live long holder: a dead or stale one would have been reclaimed inside those retries. Non-`SYNC_FAILED` acquisition errors still fail loud (cell added).
- **MINOR-R2** (resume-path marker overwrite unprobed — same class as the `beginFullSync` probe): `assertLockHeld` added before the carry-forward `writeFullSyncCheckpoint`.
- **MINOR-R3** (MINOR-5 decline reasons falsified: index-meta is DATABASE_MISMATCH evidence, not a regenerable cache, and the probe cure never touches short holders): probe added before `writeIndexMeta`. The MINOR-5 decline now covers only the index-manifest write (genuinely regenerable, rewritten every sync by any completing run).
- **MINOR-R4** (release's unreadable-guard misattributed a transient own-file read failure as theft, and the inverse trade was undisclosed): release's unreadable arm now warns as unreadable-not-theft. Owner-correction to the leg's stall claim: an EXITED holder's leftover reclaims immediately via the dead-PID arm; only a still-alive holder's leftover waits out the threshold — recorded here as the disclosed residual.
- **MINOR-R1 / NIT-R1** (E2E comment overclaimed "either probe"; `not.toContain(unflushedFile())` tautological): comment corrected to claim exactly the exit probe, with the entry probe's non-inducibility (no await between exit and next entry on the full path) disclosed in place; tautological assertion removed.
- **NIT-R2** (no cell pinned that the normal path takes the lock): inverse cell added (lock taken, bounded).
- **NIT-R3** (`.hb` temp not gitignored — surfaces in the shared tree's untracked disclosure): `.totem/sync.lock.*` gitignore line added.

**Verified benign by the leg** (recorded, no change): marker-deleted-while-lock-still-held is reachable and costs one 500ms–1s backoff, not a lost lesson; lockless-write races converge (content-hash filenames; the epoch resolves its file list before any mid-run write; no other holder mutates the lessons dir).

## Falsification-leg round 3 (scoped to the round-2 fold) — CONVERGENCE

Round 3 verified every round-2 disposition discharged (probe set over `writeFullSyncCheckpoint` closed — the flushBuffer append has ZERO awaits after its exit probe, so no gap exists; the `SYNC_FAILED` discriminant matches the real `TotemError.code` contract; the gitignore glob swallows nothing tracked) and declared **convergence: presentable for merge, nothing MAJOR-class open**. Residuals, all absorbed as honesty fixes (no semantic change):

- **MINOR-S1** (a `SYNC_FAILED` timeout does not PROVE a live long holder — executed counterexample: an undeletable corrupt lock at the path throws in 1ms with zero warnings; also the 5b1929d1 alive-PID-in-namespace case and short-hold churn): the round-2 disposition's proof claim is RETRACTED here; the deferral message and code comment no longer claim a cause ("usually another running sync"). The fallback is safe on every counterexample path — the lesson is written and the next sync indexes it — which is why this is a message fix, not a mechanism fix.
- **NIT-S1** ("indexed by it" overclaimed — the holder's file list predates our write): message now defers to the next sync only; leg verified the untracked-files union in `getChangedFiles` picks the file up.
- **NIT-S2** (the only remaining unprobed store mutations, disclosed now that the probe set is enumerated): `store.deleteByFile` at the two purge sites and `store.createFtsIndex`. Bounded and self-healing under a lost lock: a purge removes rows the thief's checkpoint claims and #2562's store-verification re-embeds them (wasted quota, never false-green — our window ends at the next flush-entry probe); the FTS index is derived and rebuilt by any completing run. No probe added — deliberate, matching the MINOR-5-class cost/benefit line.

Leg recommendation adopted: cascade stops at round 3 (strictly narrowing classes: MAJOR → MINOR-no-data-loss → message honesty); the branch goes to the bot round.
