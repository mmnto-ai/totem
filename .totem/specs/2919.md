...
- **Not followed, a repo-relative path.** A dispatch lives in the SENDER's outbox, in the cohort's usual shape a sibling repository of the workspace (a same-repo dispatch, seat to seat inside one repo, sits under the reader's own root, but `totem mail mark` and a shell read take the absolute path from any cwd either way), so a path relative to the reader's repo root is not a general answer and one relative to cwd is not copy-pastable across shells. The line renders the absolute `filePath` verbatim, the string `--json` emits, so it discloses nothing the `--json` arm does not already, and it is exactly what `totem mail mark` takes (lesson-296c72de).
...
- **Not followed, the directives' test names.** Task 2 names `should_include_resolved_filepath_in_human_listing` and `should_include_json_instruction_in_footer`; the shipped cases carry descriptive titles in the file's own convention, and the footer case does not exist because the footer arm was not taken.
...
- **Followed.** A test written first and RED on the previous renderer, the path under every listed item in both arms, the empty-inbox arms untouched, lint and the review lanes before the push.
