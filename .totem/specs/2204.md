...
| Withheld count bounded by `maxScan` window                | runtime    | truncation warning composes with the gate line; under the gate the union-directed tail is a COUNT — naming it would rebuild the exposure (leg F1 fold)                                                                         | raise `--max-scan` / reduce backlog                                                                  |
...
| Lifecycle sense under the gate                            | runtime    | #2511 suspended/retired notices still emit for withheld directed mail (name only a union SEAT already in the banner — leg F9 fold); collision registration stays excluded (names filenames; env-bound polls preserve coverage) | n/a (sense, not fault)                                                                               |
