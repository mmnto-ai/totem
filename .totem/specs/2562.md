...
- **New persisted file `.totem/cache/full-sync-checkpoint.json`** (NOT an extension of `sync-state.json` — that file's semantics stay "last _successful_ sync baseline"; existence of the checkpoint file IS the dirty marker, so no reserved keys / sentinel values in existing state):
  ...
  - `embedder: { provider, model, dimensions }` — resume-compatibility fingerprint; a RESOLVABLE mismatch ⇒ restart from zero (never mix vector spaces), regardless of work remaining. _[Amended round 4: the sole exemption is resolution FAILURE with zero work remaining — the marker clears unverified with a loud log rather than wedging the epoch behind a missing embedder.]_
...
### Falsification round 4 — dispositions (2026-08-03, convergence round, scoped to the round-3 fold)

- **MAJOR 1 FIXED** — round 3's zero-work exemption over-broadened: it skipped the identity gate on `filesToProcess === 0`, so a RESOLVABLE mismatch with zero work cleared the marker and rewrote index-meta to the new identity, laundering the DATABASE_MISMATCH evidence before the next incremental mixed spaces (leg-probed, both directions). The gate now exempts only RESOLUTION FAILURE with zero work; the missing test cell (zero-work × resolvable mismatch ⇒ restart) is locked. The data-model ruling clause above is amended to match.
- **MINOR (openai scope) FIXED** — openai now paces only multi-text calls, mirroring gemini; a configured `throttleMs` no longer taxes interactive queries there either.
- **MINOR (copy) FIXED ×3** — changeset's "indexes when the epoch completes" corrected to "on the next sync after the epoch" (the running epoch resolved its file set before the lesson existed); "corpus of any size converges in minutes" replaced with proportional phrasing in changeset + both wiki pages; the pacing comment's "single-text = query" premise widened to name the semantic-dedup per-candidate probes (sequential-RTT-bound, quota-immaterial).
- **NITs ACCEPTED** — completedFiles can grow linearly across repeated store-unreadable crash loops (self-prunes on the first readable resume; membership is set-tested); the empty-allFiles × unreadable-store edge clears retained paper progress with the marker (edge of an edge); the add_lesson deferral test asserts behavior, not the totemDir argument (production verified correct); the round-2 "throws loud with state intact" comment lives on in the gate's fail-loud arm.
- Leg's honest counterweight, recorded: with NO marker present, a provider/dimension config change followed by plain incremental sync mixes the store today — `writeIndexMeta` stamps the config identity and `verifyIndexMeta` has no callers. Pre-existing hole in the unmarked regime, out of this PR's scope — folded into the mmnto-ai/totem#2565 effective-identity issue.
