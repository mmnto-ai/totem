...
3. **`NonCompilableEntry` (Read/Write Separation):**
   ...
   export const NonCompilableEntryReadSchema = z.union([
     z.tuple([z.string(), z.string()]), // Legacy
     NonCompilableEntryWriteSchema       // Modern
   ]).transform(val => {
     if (Array.isArray(val)) return { hash: val[0], title: val[1], reasonCode: 'legacy-unknown' };
     return val;
   });
   ```
...
- If `!hasExampleHit`:
  - Check security context: if `rule.immutable === true` or pack/namespace implies `@totem/pack-agent-security`, immediately fail compilation and return a non-compilable result with `reasonCode: 'security-rule-rejected'`.
  - For standard rules: assign `rule.unverified = true`. If the parsed rule does not explicitly define `severity: 'error'`, force `severity = 'warning'`.
...
### Edge Cases & Traps
- **Security Rule Identification:** Checking `rule.immutable === true` isn't enough if the LLM drops the flag. Ensure you also check if the *lesson metadata* or `deps` indicates the rule is bound for the security pack.
...
| `CompiledRule.severity` semantics | (existing field) For `unverified: true` non-security rules, compile forces `'warning'` unless lesson explicitly authored `severity: error` (then doctor raises a flag). For `unverified: true` security rules, compile fails loudly and emits no rule. | `buildCompiledRule` / `buildManualRule` | Runtime lint, doctor | Security + unverified is unrepresentable in the output schema — compile rejects at the source. |
...
|---|---|---|---|
| Lesson has no Example Hit, non-security context | Runtime | Rule ships with `unverified: true`, severity forced to `warning` | Author adds Example Hit block, recompiles; `unverified` clears |
...
2. A non-security rule compiled without an Example Hit has `unverified: true` AND `severity: 'warning'` (even if the LLM emitted `severity: 'error'`).
