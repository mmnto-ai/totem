...
denied-on-undecidable. (3) a `gh api` invocation whose pre-path segment exceeds the
NOSEP cap (~2000 chars) is unclaimed — an adversarially padded header ALLOWs by design
(the cap is a deliberate linearity trade: a stalling hook stalls the harness), while the
under-cap dangerous form BLOCKs (codex round-3 #3 / kimi NB — RULED record, cap kept).
D1/D2 are the loud backstop for all three classes.
...
the detector is provably LINEAR (disjoint-class FLAGRUN + a bounded merge-path span;
adversarial perf fixtures assert <50 ms — the earlier nested-quantifier shape
backtracked multi-second at ~26 repeated flag groups).
