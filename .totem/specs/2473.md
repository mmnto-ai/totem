...
- Store: `.totem/artifacts/admissions/`, content-addressed (validate-on-write-out, `wx` create-exclusive, EEXIST dedup) with BOTH `createdAt` (**observability-only**) and `schemaVersion` (**writer metadata**) excluded from the address — identity is the OBSERVATION alone, so a 1.x version bump never orphans prior records on the exact-identity lookup path (re-arm leg, MINOR 3 of the first round). A record written by a NEWER major at the same address fails the tolerant load with a named upgrade-the-CLI error, never a generic corruption message. With identity fully bound, dedup collapses only _identical observations_ (same bytes, same policy, same scope) — the repeat is the same fact, so first-write-wins is sound. The former "whipsaw" accepted-limitation is **retired**: it was an artifact of wall-clock arbitration, which the resolver below no longer performs.
- **Covariate-line contract v1.1 (this clause IS the spec amendment the distributed format marker cites):** the admission form `local-lane: not-applicable (<reason>) recorded=<recordHash8> at=<createdAt>` is an ADDITIVE second shape of the versioned `local-lane` covariate contract (v1 → v1.1). Consumers discriminate the two shapes on the literal second token `not-applicable`; the verdict shape is byte-unchanged. The review-loop format block (init-template constant + all distributed twins) carries the amended marker, and the `init.test.ts` pin locks both shapes.
- **Covariate-line contract v1.2 (the amendment the distributed format marker cites — mmnto-ai/totem#2698):** BOTH existing shapes gain an APPENDED space-separated `leg: <sha8> blocking=<n> material=<n> folded=<n>` field naming the falsification-leg deposit that answered for HEAD, and `leg: none` when no deposit resolves for it. The field is composed BESIDE `renderCovariateLine` / `renderAdmissionLine`, never inside them: the v1 verdict text and the v1.1 admission text are byte-unchanged before it, so a v1/v1.1 consumer keeps parsing exactly the prefix it already parses. A THIRD head shape, `local-lane: none leg: <sha8> …`, renders when no verdict and no admission record exists for the lineage but a deposit does — the mmnto-ai/totem#2694 exhibit's cure, a diff presented with no evidence line at all. `none` is a HEAD-token value, never a value inside an existing shape, so second-token discrimination is unchanged in kind and only widens its set: `<hash8>` · `not-applicable` · `none`. `<sha8>` is the first 8 hex of the deposit's `diffSha`; `minor` is deliberately absent from the field (the round rules on blocking and material), and a FOLDED finding counts in BOTH its severity bucket and `folded` — `blocking=3 folded=3` reads "all addressed", which `blocking=3 folded=0` does not. The marker, both skill twins, the `init.test.ts` pin and this clause move together, exactly as v1.1 did.
...
- **Falsification leg round 1 (full branch, `2d102443`): 2 MATERIAL + 7 MINOR + 6 NIT — all folded, none declined** (fold commit `5f6c8672`). MATERIALs: the admission `local-lane` form broke the shipped format-v1 covariate contract with no amendment (cured: declared format v1.1 across the template constants + all four distributed twins; the `init.test.ts` pin fired on the amendment as designed); the classifier fingerprint's only table binding was an unenforced hand-maintained string (cured: `classifierId` embeds a mechanical digest of the exported `CLASSIFIER_POLICY_TABLES`, wiring test-locked). MINORs incl. version-free record identity (`schemaVersion` out of the address), exact-count disposition wording, canonicalizer digest pin, covariate corrupt-record CLI test, `--gate` in the `--estimate` incompatible list, and the changeset's compat claim scoped honestly (legacy PreToolUse case named). The leg's refuted-in-passing sweep confirmed the exit contract, stamp-removal completeness, phase ordering, hook seam, and covariate read-only contract against attack.
- **Re-arm falsification leg (fold-scoped, `2d102443..5f6c8672`): 1 MATERIAL + 5 MINOR + 4 NIT — all folded.** MATERIAL: this spec contradicted the shipped identity semantics (createdAt-only exclusion) and the distributed v1.1 marker cited an amendment this spec did not carry — both cured above (the store bullet + the v1.1 clause ARE the amendment). MINORs: `--estimate --gate` test row added; mixed generated+prose count fixture added; the schemaVersion exclusion's stale doc sites updated incl. the user-facing verification error; a NAMED newer-major load error added (an older CLI meeting a newer record never reports valid data as corruption, test-locked); the version-stability test's lookup half now genuinely crosses versions. NITs: false quiet-drops-a-read comment fixed; count-wording comment descoped; remaining "structurally unreachable" sites aligned; `<recordHash8>` placeholder unified. **Named follow-up question (pre-existing at `2d102443`, ruled OUT of this slice):** `ProjectionPolicy.sourceExtensions` is bound into the admission address but consumed by the content-hash surface, not the admission projection — over-binding that re-keys addresses on a `review.sourceExtensions` edit with no behavior change (conservative churn, not unsoundness); belongs to the admission-store reaper follow-up.
