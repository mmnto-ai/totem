...
- **Attribution (F5, corrected c.4755848293)** — Layer-B `agentSource` = a **stable actor-id**; model/backend = **separate optional metadata, never folded into the id** (so hit-rate aggregates across model swaps). Namespace = `{ cohort agent-seats } ∪ { coderabbit, gemini-code-assist, greptile }`: cohort seats → `cohort-roles.md` ids (`totem-claude`, `strategy-codex`, …); the **3 active paid review bots** → their login-stem id (de-facto home `doctrine/bot-protocols.md` + the parity-manifest `bot-review-configs` dimension). **`pr-agent`/`qodo` are dropped** (research-only; the #670 backend registry does NOT exist yet — forward-couple only IF it ships). **ADR-078 `agent_source` is NOT amended** (compliance-attribution ≠ capability-attribution).
