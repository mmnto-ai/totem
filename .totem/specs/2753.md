 ...
 *  non-empty after trimming — a whitespace-only value does not attest
 *  (mmnto-ai/totem#2753; the trim from the mmnto-ai/totem#2760 bot round). */
...
- The trailer is attested iff SOME line in the run parses AND its `reason`, `owner` and `attested` are each present and non-empty after trimming (a whitespace-only value does not attest). A trailer with no non-blank line, or a run with no full marker, is NOT attested — the blank case is the pre-existing owned-whole-file shape, which drift-repair already handles.
