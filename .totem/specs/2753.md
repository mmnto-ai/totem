...
|                                            |  ┐ the LEADING COMMENT RUN: blank
| # [lc] docs-inject extension               |  │ lines and comments, up to the
| # <!-- totem:fork reason="…" owner="…"     |  │ first COMMAND. A full marker
|      attested="2026-06-07" -->             |  ┘ anywhere in it → attested.
| sh "tools/git-hooks/pre-commit-docs-inject.sh" |  ← first command ends the run
+--------------------------------------------+
                  |
...
+--------------------------------------------+
|                                            |
| # [lc] docs-inject extension               |   ← byte-for-byte identical
| # <!-- totem:fork reason="…" owner="…"     |
|      attested="2026-06-07" -->             |
...
- The INSTALLED TIER survives a bare rewrite, on both the attested-trailer and the owned-whole path; an explicit `--strict` / `--standard` flag still wins in both directions.
