...
- `evaluateGroundingFloor` (`cli/src/commands/spec.ts`) treats a relevance failing the same predicate as floor-EXEMPT rather than signal.
...
The CAUSE is metric-specific (`OUT_OF_RANGE_CAUSE`, folded in from the falsification round, F1): squared L2 is `≥ 0` by construction, so under `l2` a breach is "a negative or non-finite `_distance`, which squared L2 cannot produce: an SDK or data fault", NOT the non-unit-norm embedder that `cosine` / `dot` really do break on. In `runHybridSearch` the tally runs over the vector leg's RAW rows, before fusion and the `maxResults` slice (F3).
