...
- `evaluateGroundingFloor(context, isInRange, floor)` (`cli/src/commands/spec.ts`) treats a relevance failing the same predicate as floor-EXEMPT rather than signal. The predicate is a PARAMETER (a static core-barrel value import in `commands/**` pulls LanceDB into every CLI startup — mmnto-ai/totem#2339, enforced), and it precedes `floor` so mmnto-ai/totem#2758's `floor?: number` can land without a required parameter following an optional one. This PR merges after mmnto-ai/totem#2758 and is rebased onto it; both edit `spec.ts`.

  The refusal effect is a LOOSENING, on one path only. `refuse` requires `bestRelevance < floor`, so a HIGH out-of-range value never defeated a refusal — it made `refuse` false by raising `bestRelevance` before, and does so by being exempt now. The arm that moves is the NEGATIVE one: a free-text run whose only signal hits carry a negative relevance (a negative `_distance` — an SDK or data fault under `l2`) used to refuse and now proceeds, with the warning printed and the value omitted from the artifact.
