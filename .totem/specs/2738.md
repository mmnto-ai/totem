...
- `packages/core/src/artifacts/grounding.ts` — The grounding-bundle builder, the FIRST consumer of a computed relevance: it decides which values reach `GroundingItemSchema` (whose `[0, 1]` bound throws at the artifact write).
- `packages/cli/src/commands/spec.ts` — `evaluateGroundingFloor`, the SECOND consumer: it decides which relevances count as signal for the refusal gate. It must read a relevance the same way the bundle builder writes one.
...
A metric with no entry is unreachable through the type; `relevanceFromDistance` calls `assertDistanceMetric` on its own argument, so a spelling that reached it past the type raises the named `TotemError` rather than a bare `TypeError` from an undefined map entry. It still never warns and never throws ON RANGE — it computes; the caller judges with `isRelevanceInRange`. A pure map, one job.

`isRelevanceInRange` is the ONE predicate both downstream consumers use, so the record and the judgment cannot disagree about the same hit:

- `buildGroundingBundle` (`artifacts/grounding.ts`) carries a relevance onto its item only when the predicate holds, and OMITS it otherwise — the item survives with its relevance absent, exactly an FTS-only hit's shape. The check must live there because `GroundingItemSchema.relevance` is `z.number().min(0).max(1)` and THROWS at the artifact write; leaving the range to the schema would turn the search layer's "warn, never throw" into a hard failure one layer down. Never clamped: a clamp would launder a fault into a plausible-looking measurement.
- `evaluateGroundingFloor(context, isInRange, floor)` (`cli/src/commands/spec.ts`) treats a relevance failing the same predicate as FAULTED — neither signal nor exemption (revised at the bot round: the build had read it as exempt). The predicate is a PARAMETER (a static core-barrel value import in `commands/**` pulls LanceDB into every CLI startup — mmnto-ai/totem#2339, enforced), and it precedes `floor` so mmnto-ai/totem#2758's `floor?: number` can land without a required parameter following an optional one. This PR merges after mmnto-ai/totem#2758 and is rebased onto it; both edit `spec.ts`.

  The refusal effect is a TIGHTENING on the fault path (revised at the bot round, Greptile P1): a faulted hit cannot raise `bestRelevance`, cannot be withheld as a measurement, and cannot save the run the way a keyword-only hit does; a run whose every hit is faulted refuses ("nothing usable grounds this run"), and a refusal beside a fault names the faulted count. The build's earlier reading had counted a fault as exempt and disclosed a loosening on the negative arm; an SDK fault is not evidence, so it may not authorize synthesis.
...
```
[Totem] <n> vector hit(s) carried a relevance outside [0, 1] under metric "l2" (sample <value>) — <OUT_OF_RANGE_CAUSE[metric]>; results are returned unchanged.
```

The CAUSE is metric-specific (`OUT_OF_RANGE_CAUSE`, folded in from the falsification round, F1): squared L2 is `≥ 0` by construction, so under `l2` a breach is "a negative `_distance`, which squared L2 cannot produce: an SDK or data fault" — a NEGATIVE one specifically, since a non-finite `_distance` is discarded before the map runs and can never reach the warning — NOT the non-unit-norm embedder that `cosine` / `dot` really do break on. In `runHybridSearch` the tally runs over the vector leg's RAW rows, before fusion and the `maxResults` slice (F3).
...
- **Epsilon Deviations**: Floating-point precision may occasionally produce a relevance slightly outside the `[0, 1]` range (e.g., `-1e-16`). The system should issue a warning as required, but must not throw an error or filter out the document. The DOCUMENT is never dropped — only the out-of-range NUMBER is, at the bundle builder, so the item is still delivered and still recorded, with its relevance honestly absent.
- **Finite numbers only**: a relevance is computed only when `_distance` is a finite NUMBER. `NaN`, `±Infinity` and non-numeric values yield NO relevance (the score falls to `_score`, else `0`) — a `NaN` is not a measurement, and `1 / (1 + -Infinity)` is `-0`, an in-range-LOOKING number that would otherwise be recorded as a real measurement of zero similarity. CONSEQUENCE, which must be named rather than discovered: a hit with no relevance is floor-EXEMPT in `totem spec`'s grounding gate, the same as a keyword-only hit, so it can no longer be excluded by the floor.
- **One predicate across surfaces**: the bundle builder's omission and the spec floor's exemption must use the SAME `isRelevanceInRange`. A floor that read a finite out-of-range value as signal would let it set `bestRelevance` and defeat a refusal on a run whose artifact carried no relevance for that hit at all.
...
Replace the hard-coded `1/(1 + _distance)` with a named-metric map keyed by the metric the store explicitly queries with, record that metric in the query and in the index manifest, and warn once per query when a computed relevance leaves [0, 1]. NOT in scope: the floor's default (mmnto-ai/totem#2727); this slice changes only which hits the floor may judge. Also out: a per-item or pre-fusion floor (`distanceRange`), any threshold, vector normalization, the embedders.

### Data model deltas

`DistanceMetric` (`'l2' | 'cosine' | 'dot'`), `VECTOR_DISTANCE_METRIC = 'l2'`, `relevanceFromDistance`, `isRelevanceInRange`, `assertDistanceMetric` in core (`store/relevance.ts`, exported). Index manifest gains `vectorDistanceMetric`. `runVectorSearch` gains `onWarn`. `evaluateGroundingFloor(context, isInRange, floor)` in the CLI — predicate before floor, per mmnto-ai/totem#2758. No config key.

The changeset carries `'@mmnto/cli': patch` alongside `'@mmnto/totem': minor`. Not because the CLI would otherwise go unreleased — the fixed group releases it regardless — but so the loosening and the floor-exemption change are disclosed in the CLI's OWN `CHANGELOG.md`, where a `totem spec` user reads them.
