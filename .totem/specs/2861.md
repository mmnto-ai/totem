...
<!-- totem-context: this section is kept CURRENT with the shipped contract (re-based on fold 3's ruling and fold 4 on 2026-09-16, after a bot round read the fold-0 text here as the contract); the fold records below are the history of how it got here, and where an older fold record and this section disagree, this section governs -->

Predicate 4 (`high-severity-inline`) gains a DISCHARGE: a bot HIGH/Major inline whose `comment.commit.oid` is the head no longer counts as "still applying" when its thread is RESOLVED and a disposition NAMES it on the same read — a non-bot reply after the root inside the thread, or a non-bot PR-level comment created strictly after the root carrying a `disposition: <root comment id> <verb>` line for THAT thread (the thread-level rule of the 2026-09-16 ruling, fold 3; the first cut's round-level read, recorded in folds 1–2, is gone). It will NOT add a comment-id override (`TOTEM_MERGE_GATE_OVERRIDE=1` stands; the draft's "option 2" is not chartered), will NOT validate the verb (the resolve-threads rule stays keyword-free; the gate reads the id, never the word), will NOT change predicate 2 (an unresolved HIGH still denies there, so the resolve stays required), will NOT touch the severity read, and will NOT issue REST.

### Data model deltas

- **The query** (`MERGE_READY_FRAGMENT`): thread `comments(first: 10)` selects `pageInfo { hasNextPage }` and, per comment, `databaseId`, `author { __typename login }` and `createdAt`; the PullRequest selects a fourth paginated connection `comments(first: 100, after: $commentsAfter)` with `pageInfo { hasNextPage endCursor }` and `nodes { author { __typename login } body createdAt }`; `SHARED_VARS` gains `$commentsAfter: String`. The exported query's sha changes, so the three PR captures are re-captured with it (the README rule); the benign corpus is not a page capture and is untouched.
- **`ThreadEntry`** gains `rootId: number | null` (the root's `databaseId`, read on the root only; the schema types the field nullable, so a root without a safe-integer id is a THREAD no line can name — it stays applying and the page stays readable), `rootCreatedAt: string` (read strictly — a root without a string `createdAt` is an unreadable thread), `humanReplies: number` (non-bot comments after the root within the read window) and `commentsComplete: boolean` (`!pageInfo.hasNextPage`; a comments connection with no `pageInfo` is unreadable).
- **`PrCommentEntry { isBot: boolean; createdAt: string; dispositions: readonly number[] }`** (new), read by `readPrComments` with the same strictness as `readReviews`: a missing connection, a missing `nodes` array or `pageInfo`, a node that is not an object, a non-null author without a string `__typename` and `login`, or a comment without a string `body` or `createdAt` is an unreadable page. `dispositions` is `dispositionedRootIds(body)`: every `disposition: <id>` line at line start (fenced lines count; lines inside HTML comments do not), the id an exact decimal followed by a blank or the line end — `01001`, `1001.5`, `1001x` and `1001-fixed` name nothing. `ReadState.prComments`, `PrPage.comments / commentsHasNext / commentsCursor` carry it across pages.
- **Bot test for an evidence comment** (`readAuthor`, both surfaces): a null author (deleted account) is NOT a bot (the reply was human when written — the resolve-threads rule, item 6 of `.totem/specs/2841.md`; unreachable for a bot under the id rule, since no bot posts the line); otherwise `__typename === 'Bot'`, or `hasBotAppLoginSuffix(login)`, or `isBotReviewerLoginExact(login)`. `__typename` is REQUIRED for the same reason resolve-threads requires it: a document that quietly stopped selecting it would fail OPEN (every App reply reading as the human answer).
- **`Discharge`** (internal): `'in-thread-reply' | 'pr-level-disposition' | 'none'`. There is no unread state: a candidate (bot-rooted, HIGH, on head, resolved) with no evidence stays applying whether its window was complete or not — a fact-side DENY at predicate 4 at both tiers, the reason naming the window when it was incomplete (fold 1 retired the `unread` arm that had routed this shape to UNEVALUABLE and so to pilot's `warn`).
- **`MergeReadyProvenanceDetail.dischargedHigh: number`** and **`dischargedBy: { inThreadReply: number; prLevelDisposition: number }`** (new, additive). `highInline` keeps its name and now counts the HIGHs still applying AFTER discharge — the number the calibration rows already cite.
...
| ----------------------------------------------------------------------------------------------------------------------------- | -------------- | --------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| the PR `comments` connection is missing, has no `nodes` or no `pageInfo`                                                      | runtime (read) | UNEVALUABLE, named (predicate 1's existing read class, shared by all four connections; its tier mapping is unchanged) | re-run; a query drift is caught by the README query-sha test                                                                                                                        |
| a thread comment without `createdAt`, or an author without `__typename` / `login`                                             | runtime (read) | UNEVALUABLE, named                                                                                                    | same                                                                                                                                                                                |
| a candidate thread's comments window is incomplete AND no evidence was found                                                  | fact           | DENY at predicate 4 at both tiers; the reason names the window                                                        | post the PR-level line naming the root id (read in full, every page) or reply in-thread within the first ten comments                                                               |
| a candidate's root instant does not parse, or its root has no readable `databaseId`                                           | fact           | stays applying: DENY at predicate 4; the reason says the id was not readable                                          | —                                                                                                                                                                                   |
| a resolved HIGH on head with no line naming it (a bare UI resolve, or a round disposition that named other threads)           | fact           | DENY at predicate 4; the reason leads with the exact line it looked for                                               | post a NEW comment carrying the line (a line edited into an older comment reads as posted then); the thread is already resolved, so the next read discharges it                     |
...
| a non-bot comment post-dates the root but carries no line naming the thread (chatter, a trigger, another round's disposition) | fact           | not evidence — the thread stays applying                                                                              | none needed: the id rule closed the round-level fail-open both legs executed (folds 1–2); the discharge still requires the RESOLVE, the seat's deliberate act after the disposition |
| a deleted-account (null author) in-thread reply, or a null-author PR-level comment carrying the line                          | design         | silent — counts as human (mirrors resolve-threads)                                                                    | disclosed in `readAuthor`; unreachable for a bot under the id rule                                                                                                                  |

No row degrades silently into a pass: every unreadable PAGE is the UNEVALUABLE class, every unreadable THREAD stays applying, and the one design limit moves only the discharge of a thread a human ALSO resolved and named.

### Invariants to lock in via tests

- A resolved bot HIGH on head with a non-bot PR-level comment strictly after its root carrying `disposition: <its root id> <verb>` is discharged: predicate 4 passes, `highInline` 0, `dischargedHigh` 1, `dischargedBy.prLevelDisposition` 1, and one stderr notice names the discharge.
- The same shape with a non-bot IN-THREAD reply and no PR-level line is discharged (`dischargedBy.inThreadReply` 1).
- A resolved bot HIGH on head with NO line naming it still denies at predicate 4, and the reason quotes the exact line it looked for, on `reason` and on `matched` at both tiers (the negative control).
- A BOT comment carrying the line, a human line PRE-dating the root, a human line naming ANOTHER thread, and a `[bot]`-suffixed login's line are not evidence (all live in the negative control); an unresolved thread with a line stays at predicate 2.
- An UNRESOLVED HIGH with evidence denies at predicate 2, and `highInline` still counts it: the resolve is required.
- A candidate whose comments window is incomplete and carries no evidence DENIES at predicate 4 at both tiers, the reason naming the window; a candidate whose evidence is found inside the window is discharged even when the window is incomplete.
...
- A deleted-account (null author) reply counts as evidence; a root without a readable `databaseId` stays applying at both tiers with `ref: high-severity-inline` and no unevaluable notice.
- The id token is the exact decimal: `01001`, `1001.5`, `1001x`, `1001-fixed` and a glued `disposition:1001` name nothing; a fenced line counts; a line inside an HTML comment (terminated or not) does not.
...
- **The line.** `disposition: <root comment id> <verb>` — one per bot-rooted thread the round answered, on its own line (fenced or not; leading blanks allowed; an HTML comment does not count; a blockquoted, listed, tabled or inline-span line is not at line start). The id is the thread root's REST comment id (`databaseId` in GraphQL; the `id=` on a `totem resolve-threads` dry-run row; the one command that prints it — `totem triage-pr` carries the id internally and prints none, the third leg's r3-f2). The verb is the round's word for it (fixed / declined / deferred / nit / extracted / held); the gate reads the id and does not validate the verb. `dispositionedRootIds(body)` is exported and the tests assert through it.
...
### Fold 4 — after the third leg (2026-09-16, deposit against `327504d7`): provenance and one design surface

The third leg returned 0 BLOCKING, 4 MATERIAL, 8 MINOR and named convergence on the gate's semantics: the id rule closes the class both earlier blocking findings named (it executed the full fail-open matrix on the built core and every arm held; both live calibration lines verify and discharge; the negative control goes red on the fold-2 code). What it found is claims about surfaces outside the gate, and one design surface. All twelve folded here.

- **r3-f1 (MATERIAL, folded)** — four surfaces said the review-reply skill "emits" the line while the template paragraph had not landed. The paragraph lands in this PR: step 2 emits one `disposition: <rootCommentId> <verb>` line per bot-rooted thread the round answered, between the per-item prose and the `local-lane:` line, with the provenance of the id and the gate's rule stated; the skill is re-stamped into `.claude/skills/review-reply/SKILL.md` and mirrored. The changeset now bumps `@mmnto/cli` too.
- **r3-f2 (MATERIAL, folded)** — `totem triage-pr` carries the root id internally and prints none; `totem resolve-threads` is the one command that prints it (`id=` on every dry-run row). Every surface corrected, the template paragraph included.
- **r3-f3 (MATERIAL, folded)** — the id began at character 161 of the deny clause and was cut from `provenance.matched`. The line now LEADS the clause ("resolved, no "disposition: <id> <verb>" line after its root and no non-bot reply …"), and the tests pin the id on `matched` for both clauses at both tiers.
- **r3-f4 (MATERIAL, folded)** — the strict `databaseId` arm refused the whole page when any root lacked a safe-integer id, and the schema types the field nullable, so a PR that should deny at predicate 1 could reach pilot's `warn`. `rootId` is now nullable: a root without a readable id is a THREAD no line can name — it stays applying (fail-closed for that thread, the reason saying its id was not readable) while the page stays readable. Fixture `synthetic-high-inline-root-id-missing.json`, asserted at both tiers with `ref: high-severity-inline` and no unevaluable notice.
- **r3-f5 (MINOR, folded)** — `Number()` had equated `01001`, `1001.5`, `1001x` and `1001-fixed` to 1001. The token is now the exact decimal (`[1-9]\d*`) followed by a blank or the line end; the matrix test pins eight lenient shapes as non-matches.
- **r3-f6 / r3-f7 / r3-f8 (MINOR, folded)** — `readThreads`' docstring and the changeset say the id is read on the root only and thread-scoped; the test header count matches the assertion (54 synthetic, 59 files); the README's instants sentence counts seven, three and one, the negative control in the second group.
- **r3-f9 (MINOR, folded)** — an unterminated `<!--` is stripped to the end of the body, the way a renderer hides it; the "nothing else names it" sentence is an inference, said so, naming the double quotes in the gate's own reason as the guard the leg could not break.
- **r3-f10 (MINOR, folded)** — with more than one HIGH applying the reason names the first and says a `totem resolve-threads` dry run lists every root id; the fold-3 record says "the first id".
- **r3-f11 / r3-f12 (MINOR, folded)** — `readAuthor` discloses the null-author rule on both surfaces and names it unreachable under the id; `dischargeOf` discloses that `createdAt` is the creation instant, so a line edited into an older comment reads as posted then and a late line goes in a new comment (as the calibration replay did).

**Convergence, named by the seat.** Three legs: the first two found the fail-open class and drove the ruling; the third executed the closed class and found only provenance. No further leg before presenting: the remaining surface is the template paragraph's text, which the third leg read in draft and found grammar-identical to the gate's regex, and which a bot round will read again. The PR leaves draft on this fold; the merge word is the operator's.
