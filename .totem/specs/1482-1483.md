...
_Contract Update (CompileTrace):_
...
_Contract Update (Doctor Config):_

```typescript
const DoctorConfigSchema = z
  .object({
...
_Contract Assumption (Compiled Rule):_
...
| `TotemConfig.doctor.staleRuleWindow: number` (new) | Minimum cumulative `evaluationCount` before a rule can be flagged stale. Default `10`. v1 cumulative-lifetime semantics; #1550 tracks the rolling-window ring-buffer upgrade behind the same key.    | User config                                              | `doctor` staleness check                     | Integer ≥ 1                                                                            |
...
| Stale rule lookup encounters a rule with absent `contextCounts`                  | Runtime   | Treat `contextCounts?.code ?? 0` as zero hits; the rule still needs `evaluationCount ≥ staleRuleWindow` before flagging | Rule exercises through normal lint runs |
...
| `doctor.staleRuleWindow` set to zero or a negative number                        | Init      | Zod `z.number().int().min(1)` rejects the config with a clear error                                                     | Fix config                              |
...
6. A rule with `evaluationCount >= staleRuleWindow` AND `contextCounts.code === 0` is flagged as stale-standard.
...
10. Config validation rejects `doctor.staleRuleWindow` of zero or a negative number.
...
1. **`evaluationCount` vs `compiledAt`-age proxy for the "exercised" gate?**
   ...
   - **Shipped:** (a) with cumulative-lifetime semantics for v1. A rolling-window ring-buffer (`RuleMetric.runHistory`) would be larger than the rest of the PR combined, with zero users as of 2026-04-18 to justify it. #1550 tracks the semantics swap behind the same `doctor.staleRuleWindow` key, so no user migration is needed when the ring buffer lands.
