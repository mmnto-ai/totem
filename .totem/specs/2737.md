...
- **Template Escaping**: Since the hook is generated from a template literal inside TypeScript, double-escapes (`\\`) are required for regex literals (e.g., `\\s*\\(.*?\\)\\s*`). A single escape will be consumed by the compiler and output an invalid regex in the raw hook.
...
- **REFLEX_VERSION**: If the template code is altered, the hook is considered out-of-date. We MUST increment the `REFLEX_VERSION` constant (in `packages/cli/src/commands/init-templates.ts`) to force clients to re-install their hooks.
...
- [ ] **Task 2: Implement Level-Aware Heading Verification**
  ...
  - Replace `isHeading(line)` with `getHeadingLevel(line)`.
  - Refactor `hasBodyAfter` to accept the parent heading's level and only terminate on levels `<= parentLevel`.
  - Ensure any level checks use `getHeadingLevel` correctly.
  - Run tests -> Verify the sub-heading defect is resolved.

- [ ] **Task 3: Implement Tolerant Heading Matcher**
  - Add the `normalizeHeading(text)` utility inside the generated hook string template.
  - Remember to double-escape any regex character classes (e.g., use `\\s` and `\\(`) to preserve them through the TS template literal compiling step.
  - Update the matching loop in `install-hooks.ts` to fallback to tolerant matching if exact matching fails.
  - Ensure a warning is logged when a tolerant match is used.
  - Run tests -> Verify parenthetical drop defect is resolved, and all positive mutations pass.

- [ ] **Task 4: Update Hook Versioning & Compile**
  - Locate `REFLEX_VERSION` in `packages/cli/src/commands/init-templates.ts`.
  - Increment the version to force a hook update for developers.
...
3. Validate that running `totem install-hooks` successfully regenerates the local `.git/hooks/pre-commit` file with the updated `REFLEX_VERSION` and the new JS matching logic.
...
Fix the strict pre-commit reader's two measured defects (a level-blind body scan; an exact-match heading test) and extend the promise to all nine headings, in that order, behind a frozen positive/negative mutation suite and the ruled falsifier over the seven recorded R3 drafts. NOT in scope: any content judgment of a draft, schema-constrained rendering (mmnto-ai/totem#2739), the reader's exit vocabulary, the DOCUMENT shape's rule, agent detection (mmnto-ai/totem#2706).
...
Per hook invocation only: the reader runs, prints, exits. No file is written. `REFLEX_VERSION` 13 → 14 marks the hook text as newer for `totem init` / `totem doctor`; consumers whose hooks carry trailing content need `totem hook install --force` (mmnto-ai/totem#2753).
