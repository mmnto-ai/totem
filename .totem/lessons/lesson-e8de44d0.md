...
## Rule
After any operation in `.strategy/`, immediately `cd /d/Dev/totem` (or the main repo root) before running git commands. The submodule is a separate git repo with its own HEAD, branches, and remote. Commands that work in the submodule context will silently produce wrong results in the main repo context.
