...
**Reproduction:** `mmnto-ai/totem#1823` R1 (GCA, 2026-05-05) — flagged 3 stderr.write call sites in `.claude/hooks/session-context.js` for both prefix-swap and throw-instead-of-write. Empirical decline grounding: `auto-context.ts:149` precedent + the file's own `process.exit(0)` outer catch + §9 line 112 (resilient continuation). Substrate-update path closed via `.gemini/styleguide.md` §6 entry.
