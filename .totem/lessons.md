...
## Lesson — Quality gate tools must never exit successfully
...
## Lesson — Regex patterns generated by LLMs from natural language
...
## Lesson — For internal, version-controlled configuration files
...
## Lesson — Do not manually edit machine-generated artifacts
...
## Lesson — Always fall back to resolving Git references
...
## Lesson — File-appending logic should implement an early return
...
## Lesson — Intentionally duplicating prompt assembly logic
...
## Lesson — Runtime escaping and sanitization are unnecessary
...
## Lesson — Resist merging similar orchestrator output handling
...
## Lesson — Diff parsing state machines must track hunk status
...
## Lesson — Local git metadata like branch names, commit messages,
...
## Lesson — While interactive users can be trusted to review
...
## Lesson — Heuristic validators designed to detect tag leakage
...
## Lesson — Sanitize LLM-generated content before persisting it
...
## Lesson — Validate that an existing git hook is a shell script
...
## Lesson — Automated review tools often have stale knowledge
...
## Lesson — Avoid adding explicit runtime guards for conditions
...
## Lesson — Moving security-sensitive regexes (like BiDi stripping
...
## Lesson — Audit initialization files and configuration schemas
...
## Lesson — Favor semantically clear naming
...
## Lesson — Implement tests that verify the presence of key sections
...
## Lesson — Custom glob matching functions must be tested
...
## Lesson — Convert large plain-text research blobs into Markdown
...
## Lesson — Use occurrence counts of opening and closing markers
...
## Lesson — In systems using "first-match-wins" resolution for file

**Tags:** configuration, architecture

In systems using "first-match-wins" resolution for file types, place specific file patterns before broader globs. This allows specialized handling of specific files within a directory without requiring complex exclusion rules in the catch-all patterns.

## Lesson — Aligning a project's own configuration with the defaults

**Tags:** maintenance, configuration

Aligning a project's own configuration with the defaults generated by its initialization commands prevents implementation drift. This ensures the repository remains a reliable reference for the tool's intended standard behavior for new users.

## Lesson — When passing LLM-generated text to CLI commands, write

**Tags:** security, cli, shell

When passing LLM-generated text to CLI commands, write the content to a temporary file and use the `--body-file` flag instead of direct string arguments. This prevents shell injection vulnerabilities and avoids complex escaping issues with untrusted or poorly formatted model output.

## Lesson — Use warnings and failure counters instead of throwing

**Tags:** architecture, error-handling, cli

Use warnings and failure counters instead of throwing errors within batch execution loops to prevent a single item failure from aborting the entire process. This pattern allows the command to be "resilient" by completing all possible actions while clearly reporting specifically what failed.

## Lesson — Explicitly truncate input data and log warnings

**Tags:** llm, performance, reliability

Explicitly truncate input data and log warnings when context sizes approach the model's token limits. Proactively managing input length prevents silent failures or API errors that occur when large datasets (like issue backlogs or documentation) exceed the LLM's effective window.

## Lesson — Manually validate LLM-generated JSON for specific types,

**Tags:** llm, validation, typescript

Manually validate LLM-generated JSON for specific types, enum values, and field existence to prevent runtime crashes from malformed model responses. For small, fixed schemas, explicit manual checks can be more lightweight and maintainable than adding heavy schema validation libraries.

## Lesson — Apply condensed lesson snippets for high-frequency, shallow

**Tags:** llm, tokens, architecture

Apply condensed lesson snippets for high-frequency, shallow commands (like triage) to preserve token windows while providing full lesson bodies to deep-analysis commands (like spec) where maximum context is critical. This tiered approach balances operational costs with the need for architectural precision.

## Lesson — Increase search pool sizes (e.g., to 20 results)
