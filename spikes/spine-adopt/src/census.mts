...
import {
  ...
  naiveScan,
  scanPattern,
  type ExpressibilityClass,
} from './lib/expressibility.mts';
import {
  ARTIFACTS_DIR,
  Checks,
  CORPORA,
...
// The scanner and the class lattice were LIFTED to `lib/expressibility.mts`
// (bodies unchanged) so `src/lower.mts` can gate on the SAME definitions —
// LOWERING.md § Lowering 4 says "only census classes `re2-clean` and
// `word-boundary` lower", and two copies of that scanner would be free to drift.
