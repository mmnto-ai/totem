...
- Updated dependencies [24af836]
- Updated dependencies [25f9783]
- Updated dependencies [a59106f]
- Updated dependencies [6720b30]
- Updated dependencies [cb8de47]
- Updated dependencies [3a8539d]
- Updated dependencies [4e2c4a2]
- Updated dependencies [b628803]
  - @mmnto/totem@2.12.0
...
- ce510fe: `totem init` now scaffolds a public `AGENTS.md` floor. When a repository has no `AGENTS.md`, init writes one: a title carrying the project name, a managed span between `<!-- totem:agents-floor:start -->` and `<!-- totem:agents-floor:end -->`, and a stub for the repository's own conventions. The span is the product-valid instruction set any consumer gets — session start (`totem status`, `totem search`, `search_knowledge`, `totem lint` before the first push, `totem orient` on a cold start), the working rules (formatter, lint as the enforcement floor, review as the advisory lane, lesson extraction after merge, never bypass a gate without a ticket, the context re-check after long edits, controller-not-implementer), the review-bot conduct, and the installed skills. It names no vendor, no private repository, no deployment and no doctrine tag. Every later `totem init` refreshes exactly the bytes of the span — the first end marker that has a start marker before it, paired with the last such start marker, so an orphan start marker above the span never widens it and an orphan end marker above it is skipped — in the line terminator the rest of the file uses, and leaves every other byte alone; extra markers outside the span (a second span, an unpaired marker) are left alone and named. A repository whose `AGENTS.md` carries no markers is never rewritten and is told once, dimly, how to adopt the span. `totem eject` removes every complete span its forward scan finds (and the file, when the span was all it held), keeping the repository's content on either side and leaving one blank line at the seam, as the reflex scrub does; stray markers it leaves behind are named with the warning that a start marker followed by an end marker is a span by definition. A marker counts only as a whole line (at most three spaces of indent, nothing else on the line): quoted in an inline code span, mentioned mid-sentence, indented four spaces as code, or inside a closed fenced block, it is prose. Below a code fence that never closes, and inside or below an HTML comment that reaches a marker (a multi-line comment ends at the marker's own `-->`, so wrapping the span in a comment does not disable it quietly), markers are ambiguous — a quotation the author never closed, a comment that swallows a marker, or a real span under a stray opener — so neither tool guesses: nothing from that line on is touched and both name the opener's line so it can be closed. The fence scan honours two CommonMark rules a quotation depends on — a closing fence carries no info string, and a fence-looking line inside an HTML comment is raw HTML, not a fence — and has two disclosed limits: it is a line count, not a markdown parser, so a fence-looking line inside any other raw HTML block is still counted (keep such lines out of `AGENTS.md`); and marker lines must end in LF or CRLF (a file with bare-CR line endings is never touched). A file that is not valid UTF-8 is never rewritten; every write to `AGENTS.md` is a temp-file-and-rename. A file that mixes line terminators around the span is converged to the terminator the rest of the file uses. (The packages share one fixed changeset group, so every `@mmnto/*` package takes the same minor bump.)

  The template is held sterile by a test in the package that ships it (`agents-floor-sterility.test.ts`): no private repository path, deployment name, operator fingerprint, doctrine tag, agent identifier, vendor name or unqualified issue reference in the span or the rendered scaffold. The same test holds this repository's own committed `AGENTS.md` to carrying the span byte-identically — the sterile floor is the init scaffold, which is the product test the sterilization issue named — and holds every public agent-instruction surface here (the floor, the two redirects, the Junie twin, the three `.claude/docs` pages) to the same lexicon plus resolvable relative links, with one declared exception: the `totem:agent-bus` marker line, whose move into a committed config file is mmnto-ai/totem#2866. `tools/public-refs-resolve.mjs` is the network half of that acceptance test: it probes the absolute URLs, qualified issue and commit refs, qualified paths and relative links a file carries, without a token, and reports the ones a public reader cannot follow (the shapes it does not extract are named in its header).
...
- ad6f3c5: The two minors deferred from the `totem resolve-threads` round (mmnto-ai/totem#2841, leg F3 and F5), folded on the PR that installs the merge-ready gate at the pilot tier on this repository, plus two scanner-parity cures the same PR's rounds found in the gate wrapper template. (The packages share one fixed changeset group, so every `@mmnto/*` package takes the same minor bump.)
  ...
  **The merge-ready gate wrapper template (`CLAUDE_GATE_WRAPPER`) closes two fail-open paths in its shell scanner**, both divergences from core's `transport-shield` scanner that the pilot-install round named: the second character of a here-string (`<<<`) no longer opens a heredoc whose body swallowed every later line (the preceding-character guard core carries), and a `#` glued to a command substitution (`$(#`) is read as the comment it is, so a `<<word` inside it opens nothing. A merge on the line after either shape is now judged; regression rows fail on the unfixed template. Re-scaffold the committed wrapper with `totem gate install <gate>` to take the fix. What remains is disclosed in the template's comment, and where the suite can lock it, locked as a miss: the executable token (a wrapper program's operand, `command -p`, an executable spelled `gh.exe` or with a path) and the projection's git reads running before the aggregate deadline — mmnto-ai/totem#2856, the strict tier's precondition; and two further divergences from core's scanner of the same fail-open class (a comment after a paren boundary; a bare heredoc delimiter outside the template's word class), whose cure is one shared scanner — mmnto-ai/totem#2857.

  **The review-reply skill's step 4 gains one clause** (all distributed surfaces re-rendered): a clean `resolve-threads --apply` run is NOT an allow verdict, and it clears the unresolved-bot-threads predicate only when no unresolved, non-outdated thread rooted by a known review bot remains — a `skip:no-evidence` row stays unresolved and, under `--apply`, makes the run exit 2, a run narrowed with `--ids` leaves its unnamed rows as `skip:not-selected` and exits 0 — because the gate re-reads the PR on `gh pr merge`, and a bot HIGH inline whose commit cannot be read makes the evaluation UNEVALUABLE once every earlier predicate passes (pilot warns, strict denies). The step now ends by reading the floor itself, `totem gate check --event merge-ready`, and reporting that verdict beside the resolve rows before the merge word is asked for.

### Patch Changes

- Updated dependencies [ad6f3c5]
  - @mmnto/totem@2.4.0
...
- Updated dependencies [7716929]
- Updated dependencies [4f208b7]
  - @mmnto/totem@2.3.0
...
- 8b8ee20: `hooks.legsOwed.enforce` — the review-leg floor's own enforcement knob, decoupled from the hook tier (mmnto-ai/totem#2771).

  **What it does.** `'block'` makes `totem legs gate` exit its derived state — `3` legs-owed with no fresh deposit, `2` could not derive — at EVERY tier, so the managed pre-push hook refuses a legs-owed push on a standard-tier install too, while the spec-evidence and shield gates stay at the repo's tier: one gate armed without arming all three, the per-arm shape the cohort asked for. `'advisory'` makes every derived gate state exit `0` at every tier, the strict one and agent seats included (a failure before the derivation still exits non-zero). Unset keeps the tier-derived behaviour byte for byte — the strict tier and agent seats block, the other tiers print the same lines and pass — so nothing changes on upgrade.
...
- f017ee4: The strict pre-commit spec-gate reader tracks fenced code blocks and no longer counts a bare `####` line as body (mmnto-ai/totem#2769; the two holes the mmnto-ai/totem#2759 round declined as pre-existing).
...
- 45d7dff: Managed git hooks reach consumers that extend them: a bare install rewrites a stale managed block in place when the trailer is an attested `totem:fork` extension, `totem doctor` compares every managed block to the canonical on every install, and neither ever downgrades a strict hook (mmnto-ai/totem#2753).
  ...
  **MIGRATION — none.** No config key, flag, file location or command name moves, and no hook is written that would not have been written before except in the two states this slice is about. Consumers with attested extensions receive the next template on their next bare `totem hook install` or `totem init`; a consumer-side `--force`-then-re-append workaround can be retired. Five things read differently — four of them output, and one behavior:
  ...
  - **The doctor's stale-hook message tail changed on every stale case**, from `— the hook reads a tree Totem does not write` to `— the managed block is stale (a newer hook template, or a config change)`, and it now names the canonical's author (`the canonical this @mmnto/cli renders for totemDir '…'`). When every flagged hook is the `non-utf8` shape the headline names that cause instead (`… cannot be kept current … — the region above the extension (shebang line and managed block) does not decode as UTF-8`), and a mixed set appends it as a second clause, because for that shape the block may be byte-identical to the canonical and "stale" would be false. A script grepping the old tail will not match.
  ...
  - **New summary lines.** `totem hook install` prints `Drift-repaired <hook> hook (managed block rewritten in place; the attested extension after its end marker carried through unchanged).` for the new action, and `totem init`'s summary gains rows for both rewrite actions where it previously printed nothing. A second new action, `skipped-non-utf8`, prints `Skipped <hook> hook: the region above its extension (shebang line and managed block) does not decode as UTF-8, so it was left byte-identical …` with the re-save-or-`--force` cure, on both commands; `totem doctor` senses the same shape with the same predicate — a bad byte on the shebang line alone counts, even when the block itself is current — and its remediation gains the matching re-save clause.
...
- 9c7f9cb: The strict spec-gate reader measures what it promises: a body ends only at a same-or-shallower heading, a promised heading matches with one trailing parenthetical dropped or differing, and all nine promised headings are required (mmnto-ai/totem#2737).
  ...
  **What changed — three rules.** A body now ends only at a heading of the SAME OR SHALLOWER level: a deeper heading neither ends it nor counts as one, so a section that opens with a sub-heading is bodied by the text under that sub-heading. A promised heading is matched EXACTLY first, and only if no line matched is it matched tolerantly, with one trailing parenthetical group stripped from BOTH sides — symmetric, so a dropped `(structural constraint)` and a differing `(required)` both match, and level-exact, because the `###` marker is part of the compared string, so `## Problem Statement` never satisfies `### Problem Statement`. And `SPEC_REQUIRED_SECTIONS` is now all nine `### ` lines of the built-in prompt, in prompt order, so the gate asks for what the command asks for. The tolerance is never silent: the pass line carries `· tolerated <promised> ~ <found>` for each heading it accepted that way, so the drift is disclosed on the commit that relied on it rather than absorbed. Trailing whitespace is settled by `trimEnd()` on the exact pass and named nowhere — it is not drift. A tolerantly-matched heading that then blocks for an empty body names both spellings (`has an empty heading <promised> (matched as <found>)`), so the cure is never a string the draft does not contain. And no echoed value can forge a second `[Totem]` line on any `/bin/sh` any more: the hook's two evidence sinks print through `printf '%s\n'` rather than `echo`. The hole was open wherever `/bin/sh` expands backslash escapes in `echo` — `dash`, which is `/bin/sh` on Debian and Ubuntu, and macOS's `/bin/sh`, a bash built with `xpg_echo` on — and inert only under Git Bash and a plain bash, so a literal `\n` inside an artifact value became a real newline in exactly the places nobody was looking. A forged value could also TRUNCATE the BLOCKED line rather than split it (`\c` under the same shells), swallowing the cure text that tells the seat what to run, so the sink fix restores the cure as well as the line count. Sanitizing could not reach either one: a backslash is printable. Alongside it `safe()` now collapses U+2028 and U+2029 as well as C0 and the DEL/C1 band, so the line also stays one line on every terminal. Both cures apply to every value the reader echoes — `anchor.ref`, `createdAt`, `anchor.sha256`, the artifact path, a bound record's resolved realpath — not only to the tolerance clause this slice adds.
...
- ba22ea1: Vector relevance is bound to a named metric: the hard-coded `1/(1 + _distance)` becomes a map keyed by the metric the store now explicitly queries with, and that metric is recorded in the query and in the index manifest (mmnto-ai/totem#2738).
  ...
  **What the floor makes of a fault.** `evaluateGroundingFloor` in `totem spec` applies the same `isRelevanceInRange` the bundle builder omits on, so one predicate governs both surfaces and the record and the judgment never disagree about the same hit — but the judgment keeps a distinction the record does not: a hit with NO vector leg (keyword-only) is floor-EXEMPT and one such hit saves the run, while a hit whose vector relevance FAILS the predicate is FAULTED — neither signal nor exemption. A fault cannot raise the best relevance, cannot be withheld as a measurement, and cannot save a run; a run whose every hit is faulted refuses outright ("nothing usable grounds this run"), and a refusal beside a fault names the faulted count. The bot round on this PR (Greptile P1) refuted the slice's earlier reading, which had counted a fault as exempt and disclosed the result as a "loosening on the fault path": an SDK fault is not evidence the way a keyword match is, and it must not authorize synthesis. So the refusal is TIGHTER than before this slice on the fault path, not looser — a free-text run whose only hits carry a negative relevance (a negative `_distance`, an SDK or data fault under `l2`) refuses, and says why. The floor's own default and calibration are untouched (mmnto-ai/totem#2727); this slice changes only which hits the floor may judge, and how.
  ...
  Worth naming as NOT fixed here: the MCP `search_knowledge` reader still selects relevances with a bare `typeof === 'number'` check, so it would still read a `NaN` or an out-of-range value as signal. That is pre-existing behavior on a surface this slice does not touch (`packages/mcp/src/tools/search-knowledge.ts`, which has its own PR open), and it is a follow-up rather than a widening of this one. Also not fixed here: `totem spec` constructs its primary and linked stores without a warning sink (`LanceStoreImpl`'s `onWarn` defaults to a no-op), so the search layer's out-of-range tally produces no visible warning on that path — the refusal text therefore says "tallied", not "warned", and a CLI-wide warning sink is a follow-up, since the same silent default sits under every command's store construction.
...
- 12250e8: Lessons are delivered again: every lesson retrieval asks the store for `type = 'lesson'` through one helper, and the partition that could never fire is deleted (mmnto-ai/totem#2735).
  ...
  **MIGRATION — none.** No config key, flag, output shape or file location moves. Two things look different. The `Found: … N lessons` line, and the prompt section behind it, are no longer empty on an index that holds lessons. And `totem spec`'s unanchored-topic refusal gains a second form: when a free-text run retrieved lessons but no specs, sessions or code, it now says `0 grounding hits (specs, sessions, code)` and names the lessons it did not judge, so the refusal does not read as a contradiction of the count line above it. With no lessons retrieved that refusal is byte-unchanged. A project with no indexed lessons still reports `0 lessons`; run `totem sync` to index them. Worth naming: the lesson pool passes through no relevance gate and lands under a HARD CONSTRAINTS header, so on an index this size it saturates its cap on every run. A relevance gate for lessons is the mmnto-ai/totem#2727 floor arm's question.
...
- 01a9c74: The falsification-leg floor gets a reader: `totem legs deposit` records what a leg read, `totem legs gate` answers whether THIS head was read, and the strict pre-push hook blocks a legs-owed push carrying no fresh deposit (mmnto-ai/totem#2698).
  ...
  **REACH — the strict arm's live population is small, and the gate judges nothing about findings.** Until mmnto-ai/totem#2706 is ruled, the strict arm fires only on installs that set `TOTEM_HOOK_TIER=strict` and on Cursor seats; this repo's own `tools/pre-push` ships tier `standard`, so this repo's pushes print the advisory line and exit 0. The review-loop and review-reply skill blocks and the `totem legs` help say "leg deposit", not the internal spelling this note uses: consumer surfaces carry no review-process vocabulary (mmnto-ai/totem-strategy#619), and a test now holds that line on those two blocks, the `totem legs` command block and the reflex template. It is NOT a sweep of every distributed surface — `totem init` ships other skills, and `totem help --all` other descriptions, that still carry the vocabulary; that sweep is mmnto-ai/totem-strategy#619 item 3. And the floor is deliberately narrow: the gate asks only whether a leg READ this head. It never judges a finding's severity or its disposition — those counts ride the covariate line for the round to rule.
...
- 8d5e269: `totem spec` publishes what it was ANCHORED on, refuses an unanchored topic it cannot ground, binds a hand-authored design record with `--from`, and the strict pre-commit gate reads that anchor (mmnto-ai/totem#2700).
  ...
  **New artifact fields (`@mmnto/totem`).** The run artifact now publishes the discriminator instead of leaving it to be inferred:
  ...
  Every field is optional, so artifacts written before this version parse byte-for-byte. The written `schemaVersion` moves `1.2.0` → `1.3.0` — the observable marker for the widening; readers still accept any `1.x`.

  Because `relevance` enters `grounding.hash`, `totem artifact compare`'s `sameGrounding` would have narrowed from "the same items grounded both runs" to "the same items measured identically": when BOTH artifacts carry a bundle it now compares the bundles with `relevance` stripped (canonical order preserved) alongside `provenanceSummary`, and falls back to the recorded hash only when either side has no bundle. The strict signal is not lost: the comparison additionally publishes `groundingHashA` and `groundingHashB` — each side's recorded `grounding.hash`, verbatim — so `totem artifact compare`'s JSON still exposes exact hash equality beside the looser verdict. `totem artifact rerun` carries the source's `anchor` and `floor` onto the rerun — the rerun makes no new grounding claim, and the anchor is part of that identity.
...
- 2e389e3: The managed git hooks now render the CONFIGURED `totemDir` (mmnto-ai/totem#2692).
...
- 519010f: Silent failure → loud on two of the CLI's own coordination surfaces (mmnto-ai/totem#2685, mmnto-ai/totem#2690).
  ...
  **Strict pre-commit gates on spec evidence (mmnto-ai/totem#2690).** The agent-strict pre-commit block gated on `.totem/cache/.spec-completed`, which no CLI path has ever written — every fresh checkout or `totem wt create` worktree ran the command the hook named and was still blocked. The block now passes on what `totem spec` actually writes: a mmnto-ai/totem#2100 run artifact under `.totem/artifacts/runs/` whose top-level `admission.runMetadata.caller` is `spec` (written on every successful run, `--fresh` included), read JSON-aware in the hook via `node -e` — a substring match would have accepted a `review` artifact whose prompt merely quotes the key (the review of this very change would have). The former hand-set marker `.totem/cache/.spec-completed` is **retired outright** — no CLI version ever wrote it, so there is nothing to stay compatible with (operator ruling: no legacy shims while there is no hard consumer); a checkout without a spec run artifact blocks until `totem spec` runs, and the message says so. A reader that cannot run (no `node` on PATH, a crash) is reported distinctly from missing evidence and still fails closed. The hook prints `[Totem] spec evidence: <artifact> (<createdAt>, N days old)` on every strict commit so a stale pass is visible; a freshness rule is a separate, named follow-up. `tools/pre-commit` and the `hooks.tier` schema doc move in lockstep; the repo's pre-managed-era, unregistered `.gemini/hooks/BeforeTool.js` — a second reader of the dead marker — is deleted rather than kept in step. Consumers pick up the new hook text on their next `pnpm install` (prepare) or `totem hook install`.
...
- aba7c78: fix(smoke-gate): `runSmokeGate` evaluates § Design 8 `requires:` with the runtime's own evaluator, so a `requires:`-bearing record's `good` example goes silent for the right reason and `totem rule test` / the cert-path preimage differential can pass such records (mmnto-ai/totem#2678).
...
- 180e0d5: **BREAKING (authored envelope):** `dslSource`, `declaredEngine`, and inline fixture `preimageSource` are removed from `.totem/spine/authored-rules.yaml` entries and rejected by name with a migration message; entries reference a `.totem/rules/<slug>.rule.yaml` record instead. No such envelope exists in the cohort; `totem rule author` requires either a frozen split artifact or a legacy free-text `splitRef` header, neither of which exists downstream.
...
- bd889f4: feat(skills): crown-gate + S0 seat-identity fold into the distributed skill constants (mmnto-ai/totem-strategy#488 R-table; operator-ruled 2026-08-13/14)
...
- 6708d16: fix(review): deterministic skips are admission verdicts — disposition record + `--gate` mapping, and the clean-tree stamp is removed (mmnto-ai/totem#2473)
  ...
  **`--covariate` is admission-aware:** a not-applicable current state resolves the exact-identity admission record (deterministic — never wall-clock arbitration across record families) and renders `local-lane: not-applicable (<reason>) recorded=<recordHash8> at=<createdAt>`. Under content-address dedup a repeat identical skip renders the FIRST observation's `at=` timestamp — that is the record's own truth, not staleness of the check. **This admission form is a versioned addition to the `local-lane` covariate contract — format v1.1** (the #2473 design § Implementation Design is the spec amendment): the review-loop/review-reply skill templates and their distributed copies are amended in this release, and any pilot-side parser keying on token 2 discriminates the two shapes on the literal `not-applicable`.
...
- 65a494a: feat(mail): multi-seat identity gate + `--as <seat>` selector + `--all-seats` union view (mmnto-ai/totem#2204)
...
- Updated dependencies [e905963]
- Updated dependencies [05edd72]
  - @mmnto/totem@1.116.0
...
- 92e2317: Ship the Gemini CLI session briefing as `.gemini/hooks/SessionStart.cjs` instead of `.js` to stop a silent fail-open (the defect reported on mmnto-ai/totem#2488), closing the scope-out the BeforeTool fix (mmnto-ai/totem#2481) deliberately left open.
...
- b5dbfe9: `verify-lockfile-sync` now fails loud when a lockfile diff silently removes a package whose pin is still declared: a failed optional-dependency fetch (e.g. an expired registry token) makes `pnpm install` drop every `pnpm-lock.yaml` entry for a working dep while exiting 0, and the resulting lockfile is internally consistent enough to pass `--frozen-lockfile` CI. The gate now parses the lockfile diff for fully-removed packages, confirms the package resolves nowhere at HEAD (section-aware — peer metadata never counts as a resolution), verifies the pin is still declared in a dependency block of a workspace importer manifest, and blocks the push with an auth-check + `pnpm update <pkg>` recovery hint (`pnpm install --lockfile-only` false-reports "Already up to date" on this class).
...
- Updated dependencies [72c8fbf]
- Updated dependencies [c79cfbf]
  - @mmnto/totem@1.106.0
...
- 6cd4847: fix(lint): corpus-bearing repos hard-error when compiled rules are unloadable
...
- b8aa74a: fix(review): deterministic skip paths no longer stamp the push-gate cache (#2466)
...
- Updated dependencies [55bdc44]
- Updated dependencies [c70f3f1]
  - @mmnto/totem@1.103.0
...
- Updated dependencies [4368f18]
- Updated dependencies [f65e454]
  - @mmnto/totem@1.102.0
...
- Updated dependencies [f21fbb8]
- Updated dependencies [296f0d0]
- Updated dependencies [64c5716]
  - @mmnto/totem@1.101.2
