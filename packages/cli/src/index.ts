...
const mailCmd = program
  ...
  .addHelpText(
    ...
    [
      ...
      '',
      "Each listed unread item carries a `read:` line — the dispatch's absolute",
      'path, the same `filePath` the --json output emits (mmnto-ai/totem#2919).',
      'Open it there and pass it to `totem mail mark`, rather than listing the',
      "sender's outbox to find it: that outbox holds the sender's dispatches to",
      'every seat, not only yours.',
      '',
