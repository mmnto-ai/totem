...
0. **Crown gate — derive FIRST (Prop 305 §8; operator-ruled 2026-08-13, distributed per the mmnto-ai/totem-strategy#488 R-table).** Read the \`totem:agent-bus\` marker in this repo's AGENTS.md. **No marker at all = un-bussed** — the Prop 305 contract is not adopted in this repo (the parity sensor's honest-absent state): this gate does not apply; run your full signoff. Marker present: derive \`primary=\` fresh — that seat is the crown. Marker present but no \`primary=\` = crown **vacant**, a legitimate state: surface upkeep stages/defers, mail the operator, and no seat self-promotes (transfer is the operator's word, Prop 305 §8.1; vacancy legitimacy §8.4b). If you are NOT the crown, your signoff is seat-anchored + own-lane: skip the surface-upkeep MUTATIONS — derived-cache/corpus rebuilds, GH board writes, cohort-wide upkeep walks, and step 4's shared-tree branch cleanup (crown-scoped there by name) — while every OTHER managed step (1–3, 5–6) still runs for every seat, and read-only derivation stays open to all: derive, then report staleness to the crown by mail. **Journal rule, every seat:** restate your OWN chain's still-open \`owed:\` lines only — lines where YOU are the waiter and \`@holder:\` names the counterparty who owes (naming yourself as \`@holder:\` makes a note-to-self the parser silently skips) — and never another seat's carryforward: the parser keys each edge's waiter to the journal-owning seat, so a copied line mints a live foreign edge in YOUR chain (a line self-held at its origin is a skipped note-to-self THERE and a LIVE edge in yours) that persists until your own next journal drops it. (Canonical rulings home: mmnto-ai/totem-status#127.)
