...
2. **Write a journal entry to the per-repo orchestration path.** Filename convention: \`<model>-NNNN-<short-topic-slug>.md\` (e.g., \`claude-0057-phase-4-resolver-shipped.md\`).
   ...
   **Resolve the path two steps:**
   ...
   This table is a RENDERING of the roster of record — \`doctrine/cohort-roles.md\` § 1 in mmnto-ai/totem-strategy — and it rots whenever a seat is added (the two Kimi cells above read "not seated" for two months after \`strategy-kimi\` and \`lc-kimi\` were seated; mmnto-ai/totem#2865): \`totem mail --derive-seat\` is the derivation, the roster is the source, and neither is overridden by a cell here. Seat discovery is dir-derived (mmnto-ai/totem#2141): any \`.totem/orchestration/<agent-id>/\` directory registers that seat for this repo, UNIONED with the basename map above so roster siblings stay visible on fresh clones where the gitignored tree is partial (precedence: \`TOTEM_SELF_AGENT\` env > \`config.json\` \`host_agents\` > seat dirs ∪ basename map). Override hook: a \`host_agents: string[]\` field in \`.totem/orchestration/config.json\` still **replaces** the derived answer — but omitting a PRESENT seat dir attaches a loud warning naming the omitted seat (the dir is the registration; config-exclusion is not a decommission mechanism). The returned list of agent-ids is used by consumers (e.g., \`totem mail\`) to filter cross-repo handoffs — messages addressed to any agent-id in the list belong to this repo's session.
