...
export const REFLEX_VERSION = 11;
...
2. **Before Implementation:** Run \`totem spec <issue-url-or-topic>\` to retrieve related context (lessons, specs, code) before writing code. Treat any generated plan as one retrieval input, never the contract — derive the actual design from primary sources (the issue, the code, project doctrine). Under the strict hook tier — which AI agents get automatically — this is REQUIRED, not optional: the pre-commit hook blocks until the checkout carries a \`totem spec\` run artifact (\`.totem/artifacts/runs/*.json\` whose top-level \`admission.runMetadata.caller\` is \`spec\`; the legacy hand-set \`.totem/cache/.spec-completed\` marker is honored second) and prints the evidence it found (mmnto-ai/totem#2690).
