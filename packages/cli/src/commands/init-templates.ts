...
2. **Write a journal entry to the per-repo orchestration path.** Filename convention: \`<model>-NNNN-<short-topic-slug>.md\` (e.g., \`claude-0057-phase-4-resolver-shipped.md\`).
   ...
   **Resolve the path two steps:**
   ...
   a. **Identify your agent-id** from the current repo's basename. The hardcoded map (Proposal 282 § Scope item 3 — the prose twin of \`COHORT_AGENT_MAP\` in \`@mmnto/totem\`, which a test holds it to; the roster of record is \`doctrine/cohort-roles.md\` § 1 in mmnto-ai/totem-strategy):
   ...
   This table is the BOOTSTRAP FALLBACK, not the roster: it carries three of the roster's five vendor columns (\`codex\` and \`agy\` seats register through their seat dirs) plus two archived repositories ADR-106 still names, and it rots whenever a seat is added — the three Kimi cells above read "not seated" for weeks after \`strategy-kimi\`, \`lc-kimi\` and \`status-kimi\` were seated (mmnto-ai/totem#2865). The Kimi column leads the core map, which still carries the Claude and Gemini pair only (mmnto-ai/totem#2875): a Kimi seat is visible where its seat dir exists, and on a fresh clone with no dirs a dispatch addressed to it is not served until that propagation lands. \`totem mail --derive-seat\` is the derivation and no cell here overrides it. Seat discovery is dir-derived (mmnto-ai/totem#2141): any \`.totem/orchestration/<agent-id>/\` directory registers that seat for this repo, UNIONED with the basename map above so roster siblings stay visible on fresh clones where the gitignored tree is partial (precedence: \`TOTEM_SELF_AGENT\` env > \`config.json\` \`host_agents\` > seat dirs ∪ basename map). Override hook: a \`host_agents: string[]\` field in \`.totem/orchestration/config.json\` still **replaces** the derived answer — but omitting a PRESENT seat dir attaches a loud warning naming the omitted seat (the dir is the registration; config-exclusion is not a decommission mechanism). The returned list of agent-ids is used by consumers (e.g., \`totem mail\`) to filter cross-repo handoffs — messages addressed to any agent-id in the list belong to this repo's session.
