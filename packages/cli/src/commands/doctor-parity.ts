  ...
  switch (contractId) {
    ...
    case 'agents-skills':
      ...
      return templates.distributedSkills.map((s) => ({
        ...
        // the repository root is the remedy; the byte-copy stays named as the
        // hand remedy for a checkout that cannot run init. Descriptive prose,
        // not a literal command (#2559 cross-bot round): `cp` is not
        ...
        installHint:
          ...
          `which writes the twin wherever an .agents/ directory exists (mmnto-ai/totem#2899)`,
      }));
