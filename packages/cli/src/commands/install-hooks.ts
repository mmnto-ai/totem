...
const tolerated = [];
// Fences (mmnto-ai/totem#2769): a line whose trimmed start is three backticks
// or three tildes opens a fence, and the same marker closes it. A fence
// DELIMITER line is neither a heading, a match candidate, nor body; a line
// INSIDE a fence is neither a heading nor a match candidate, but it IS body
// when non-blank — a section whose content is one code block is a written
// section, while a promised heading quoted inside a fence (the skeleton pasted
// as an example) is not the section it names.
...
for (let i = 0; i < lines.length; i++) {
  const lead = lines[i].trimStart().slice(0, 3);
  const marker = [TICK3, TILDE3].indexOf(lead) > -1 ? lead : "";
  if (fenceOpen.length < 1 && marker.length > 0) { fenceOpen = marker; fenceLine.push(true); inFence.push(false); continue; }
  if (fenceOpen.length > 0 && marker.length > 0 && [fenceOpen].indexOf(marker) > -1) { fenceOpen = ""; fenceLine.push(true); inFence.push(false); continue; }
  fenceLine.push(false);
  inFence.push(fenceOpen.length > 0);
}
// A line of only # characters and whitespace (mmnto-ai/totem#2769): not a
// heading (no text) and not body either — it is the skeleton of one.
...
function hasBodyAfter(start, level) {
  for (let i = start + 1; i < lines.length; i++) {
    ...
    if (n > 0) continue;
    if (fenceLine[i] || bareHashes(lines[i])) continue;
