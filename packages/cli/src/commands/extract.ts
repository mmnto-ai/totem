...
async function localExtractCommand(options: ExtractOptions): Promise<void> {
  ...
  // 10. Dedup, flag, select, persist — same as PR extraction
  ...
  log.success(TAG, `Total: ${flaggedLessons.length} novel lesson(s) from local changes`);
  ...
  if (options.dryRun) {
    ...
    for (const lesson of flaggedLessons) {
      ...
      if (lesson.scope) console.log(`  Scope: ${sanitize(lesson.scope)}`);
  ...
  console.log(`\nExtracted ${sanitizedLessons.length} lesson(s) from local changes:`);
...
export async function extractCommand(prNumbers: string[], options: ExtractOptions): Promise<void> {
  ...
  if (options.local) {
    ...
    await localExtractCommand(options);
    return;
  }
