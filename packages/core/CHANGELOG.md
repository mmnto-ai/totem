...
- ee72a24: Lower parsed Prop 310 rule records into compiled rules, add their compiled homes on `CompiledRuleSchema` (`excludeGlobs`, `requires`, `examples`, `language`, `verificationShadow`, `recoveryHint`, `curation` — all optional), and evaluate the § Design 7 two-array glob scope and § Design 8 `requires` two-pass at lint time (slice 2 of the Prop 310 build).
- e22cabf: Add the Prop 310 V1 record-grammar parser for `.totem/rules/*.rule.yaml` plus its § Design 14 conformance suite (slice 1 of the Prop 310 build).
