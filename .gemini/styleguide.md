...
- **§ 2 naming terms in documentation prose.** The metaphor-heavy-name ban (`spin`, `anchor`, `kick`, `dream`) governs IDENTIFIERS in shipped code — § 2's DO-USE list is entirely identifiers. Do not extend it to prose in internal records (`.totem/specs/**`, dispatches, journals); "empirical anchor" in a spec is a sentence, not a name. (Declined on mmnto-ai/totem#2162.)
