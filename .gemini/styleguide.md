...
- **Session-start hook stderr diagnostics — `[Totem Error]` prefix and `throw` over `process.stderr.write` in degradation-path catch blocks.** Do not suggest changing `process.stderr.write('[<hook-name>] ...')` to `[Totem Error]` prefix, or `process.stderr.write(...)` to `throw new Error(...)`, in session-start hook catch blocks (e.g., `.claude/hooks/session-context.js`). Two reasons:
