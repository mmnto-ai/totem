...
| --------------------------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `issue`, built-in prompt          | the draft (`output.content`)          | **TEMPLATE** — every heading the command promises (all nine since mmnto-ai/totem#2737, level-exact), each followed by a non-blank, non-heading line before the next heading of the same or shallower level; a deeper heading neither ends the body nor counts as one; a heading inside a fenced code block is neither a heading nor a match, and a line of only `#` characters, outside a fence, is neither a heading nor a body (mmnto-ai/totem#2769) |
| `issue`, overridden system prompt | the draft (`output.content`)          | **DOCUMENT** — at least one markdown heading followed by a non-blank, non-heading line, read through the same fence state (mmnto-ai/totem#2769)                                                                                                                                                                                                                                                                                                        |
| `record`                          | the RECORD's bytes, re-read from disk | **DOCUMENT** — the same fence state; a record whose only headings sit inside fences is cured by editing the record — a real heading with a line under it; the hook's line still names the `totem spec` cures first, which re-bind the same bytes and block again (mmnto-ai/totem#2769)                                                                                                                                                                 |

Fences follow CommonMark (mmnto-ai/totem#2769): a line indented at most three spaces whose first run is three or more backticks or tildes opens one (a backtick run only when the rest of the line carries no backtick), the closer is the same character at least as long with nothing else on the line, and an unclosed fence runs to the end. A delimiter line is neither a heading, a match candidate, nor body; a line inside a fence is neither a heading nor a match candidate, but is body when non-blank unless it is a promised heading — at any indentation, in the exact or the tolerant spelling — so a section whose only content is a code block is a written section, and a section whose only content is the quoted skeleton is not. A promised heading that exists only inside a fence — at any indentation, in the exact or the tolerant spelling — blocks as `is missing heading <h> — it appears only inside a fenced code block opened at line N`, followed by the cure that fits (close the fence when it runs to the end of the draft; write the section outside it when it is closed), and a section emptied by these rules names every rule that emptied it (`— nothing under it counts as body: a promised heading quoted inside a fenced code block; a line of # characters; an empty fenced code block`, whichever apply). So the cure is never a heading the draft contains.

A promised heading whose trailing parenthetical is dropped, or differs, still matches: the reader compares exactly first and only then with one trailing parenthetical group stripped from both sides, so the section a draft actually wrote is not read as absent over its own punctuation. The tolerance is never silent — the evidence line names each one it used, as `· tolerated ### Verification (MANDATORY — do not skip) ~ ### Verification`. The heading marker is part of the comparison, so the level is still exact, and trailing whitespace is settled before either pass and named nowhere. On the block path the same disclosure appears inline, so the cure is never a string the draft does not contain: `has an empty heading ### Verification (MANDATORY — do not skip) (matched as ### Verification)`.
...
**Two arms over the same evidence, and a knob** (mmnto-ai/totem#2771; the `legs-gate` parity row). The doctrine names two enforcement arms and a repo adopts one. The **hook arm** is the strict tier above: the push itself is refused. The **CI arm** runs `totem legs gate` as a hard step of the repository's lint workflow over deposits **committed** under `.totem/artifacts/legs/` — the directory is re-included from the artifacts ignore, so a leg of record travels with its pull request — and turns the check red on exit `3` or `2`; whether red blocks a merge is branch protection. That arm holds whatever the local tier is and `--no-verify` never reaches it. A consumer that installs `@mmnto/cli` from npm can call the bin (`pnpm exec totem legs gate`); a workspace that BUILDS the CLI in the same job cannot — pnpm links bin shims at install time and skips one whose target does not exist yet, so the step invokes the built entry directly (`node packages/cli/dist/index.js legs gate`, which is what this repository's workflow runs). Independently of the tier, `hooks.legsOwed.enforce` arms or softens this one gate: `'block'` makes the gate exit its derived state at every tier, so the managed hook blocks a legs-owed push on a standard-tier install while the spec-evidence and shield gates stay at the repo's tier; `'advisory'` makes every derived state exit `0` everywhere (a failure before the derivation still exits non-zero); unset keeps the tier-derived behaviour. The hook never reads the knob — it reads the exit code the gate has already mapped through it — so `3` and `2` block on every tier (under the advisory flag they only ever come back when the knob says block), and only a failure before the derivation stays strict-only. The gate prints one extra line when the knob is set, `[Totem] legs: hooks.legsOwed.enforce = block`, under either tier, so a block names the key that caused it. See [Configuration Reference](config-reference.md) § The Enforcement Knob.

Two disclosures:

- **Reach.** Exactly as with the pre-commit spec rule: until agent detection for Claude Code is restored (mmnto-ai/totem#2706), the strict arm fires only where `TOTEM_HOOK_TIER=strict` is set and on Cursor seats. Totem's own repo ships `tools/pre-push` at tier `standard` with the knob unset, so its local pushes print the advisory line and pass — its enforcement is the CI arm: `totem legs gate` is a hard step of its lint workflow, and `Totem Lint` is a required check on `main`. One branch of this repository is outside that arm: the Version Packages PR that `changesets/action` rebuilds on every push to main (`changeset-release/main`, same repository, exact name) deletes the consumed `.changeset/**` files and authors nothing, so the step is skipped there with a notice, and the `totem lint` step still runs on it (mmnto-ai/totem#2779).
