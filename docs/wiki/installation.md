...
- **Fully Supported:** `totem init`, `totem lint`, `totem hooks`, `totem doctor`, `totem status`, `totem rule` commands.
- **Excluded:** `totem review`, `totem lesson compile`, `totem lesson extract`, `totem sync`, `totem spec`, `totem triage` (These will exit with code `78` and prompt you to use the full `npx @mmnto/cli` version).
