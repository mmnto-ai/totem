...
- **The `AGENTS.md` floor:** when the repository has no `AGENTS.md`, init writes one — a title, a managed span between `<!-- totem:agents-floor:start -->` and `<!-- totem:agents-floor:end -->` carrying the vendor-neutral instruction set (session start, working rules, review-bot conduct, the installed skills), and a stub for your own conventions. Every later `totem init` refreshes exactly the span and leaves every other byte alone. An existing `AGENTS.md` without the two markers is yours and is never rewritten; add the marker lines where the managed span should sit and re-run `totem init` to adopt it. A marker counts only as a whole line (at most three spaces of indent, nothing else on the line), so quoting one in an inline code span, mid-sentence, indented four spaces as code, or inside a closed fenced code block is prose. Below a code fence that never closes, markers are ambiguous and both tools leave everything under that fence alone and name its line, so close stray fences. The fence scan reads a quotation the way a renderer does (a closing fence carries no info string; a fence-looking line inside an HTML comment is not a fence) but it is a line count, not a markdown parser: a fence-looking line inside any other raw HTML block counts as a fence, so keep such lines out of `AGENTS.md`; marker lines must end in LF or CRLF; a file that is not valid UTF-8 is never rewritten; and a file that mixes line terminators around the span is converged to the terminator the rest of the file uses. A start marker followed by an end marker is a span by definition, so never leave stray markers around your own text. A multi-line HTML comment that reaches a marker swallows it (the comment ends at the marker's own `-->`), so wrapping the span in a comment does not disable it quietly: both tools name the comment's line and touch nothing from it on; remove the span or run `totem eject` instead. `totem eject` removes every complete span (and the file, when the span was all it held), leaving one blank line at the seam and naming any stray marker it leaves behind.
...
- **Stray Markers (sensor row, no gate):** every `.totem/` directory under the repository toplevel that is not a root marker (the configured `totemDir`, matched on its trailing segments when it has several, and the literal `.totem` the mail verbs keep their store under), split by what git tracks — minted residue such as `packages/cli/.totem/temp` or a phantom `apps/<x>/.totem/orchestration/` warns with the remedy, a committed fixture's `.totem` is named and passes, a marker whose tracking git could not answer is named as unchecked with git's own error, and a nested repository's own marker is neither (mmnto-ai/totem#2938). The probe runs with git's location variables scrubbed, so a hook's `GIT_DIR` cannot point it at another index. Until that fix a reader verb run beneath one resolved it as the repo root; the toplevel now wins the walk, and the row names what is still minting them. A sweep its bounds stopped, or that skipped an unreadable directory, says so rather than claiming a clean tree.
...
### `totem resolve-threads <pr-number>`

Resolves the bot review threads a round has already dispositioned (mmnto-ai/totem#2841) — the lawful way past the merge-ready gate's predicate 2, which denies a merge while an unresolved, non-outdated bot-rooted thread is open. Zero LLM, **dry-run by default**: it prints one row per bot-rooted thread carrying that thread's REST root comment id and a verdict (`resolve`, `skip:already-resolved`, `skip:outdated`, `skip:no-evidence`, `skip:not-selected`), and mutates only under `--apply`. A thread is resolved ONLY with evidence, which is **an in-thread reply from a non-bot OR any non-bot PR-level comment created after that thread's root comment** — either suffices, and the PR-level arm is not keyed to the round-disposition comment specifically (no deterministic reading distinguishes a disposition from other human prose). One consequence worth knowing: on a re-invoked round the operator's own trigger comment is non-bot evidence for every thread it postdates, which is why the round disposition is posted BEFORE this verb runs. A `skip:no-evidence` row is never resolved under any flag. Two different tests, deliberately: a row is printed for a thread ROOTED by one of core's four review-bot identities — the same closed set the merge-ready gate's predicate 2 denies on, so the verb clears exactly the threads that gate reads and no others. Who counts as a HUMAN REPLY is the wider test: a bot is anything GitHub calls an App (GraphQL `author.__typename: Bot`, REST `user.type: Bot`), any `[bot]`-suffixed login, or one of core's review-bot logins — so a `github-actions[bot]` or Copilot-reviewer comment is never mistaken for the human answer. `--ids <comma-separated REST root comment ids>` narrows the batch and an unmatched id aborts before anything is resolved. `--json` emits one document: on success `{repo, pr, apply, rows, summary}`, on a failure `{error, rows, exitCode}` (`rows` is `[]` when the failure predates the plan). The verb never posts a comment, a reply or a review, and marks a row applied only when the mutation answers `isResolved: true`. Exit codes: `0` plan printed / applied clean, `1` a read that did not complete (nothing resolved) or `gh` absent, `2` an unmatched id, a failed mutation, or a selected thread skipped for want of evidence under `--apply`.
...
An issue input takes three forms: a bare number (`2929`, resolved against this repository, or against every repository under `config.repositories`), `owner/repo#N`, or an issue URL of the form `https://<host>/<owner>/<repo>/issues/<n>`. The URL rule is exact: the path before `/issues/` is exactly `owner/repo`, the number ends its path segment (a query, a fragment or a slash may follow it; `/issues/7abc` is not issue 7), and the host is lower-cased with github.com's `www.` alias folded (`github.com` gives `owner/repo`; another host gives `host/owner/repo`, the form `gh --repo` takes, so a GitHub Enterprise host works, a port or user info passed verbatim; GitLab's `/-/issues/<n>` form is refused by name, while a GitLab URL in the legacy `/issues/<n>` form is indistinguishable from an Enterprise host and fails at `gh`). A URL or a qualified ref names its repository, and the fetch runs against that repository, never the working directory's. Every `http(s)://` input is an issue URL or an explicit refusal that names what the input got wrong and the accepted forms — never a free-text topic, and never read as `owner/repo#N` (so `…/issues/5#7` is refused for its path, not taken as issue 7 of a URL); the refusal runs before the config resolves and before the index connects. A URL without a scheme is a topic. The anchor's `ref` keeps the input as typed. Anything else is a free-text topic. The default draft path is `.totem/specs/<number>.md` for a bare number and `.totem/specs/<owner>_<repo>-<number>.md` (lower-cased; a host joins as `<host>_<owner>_<repo>`) when the input named its repository — the join keeps `a-b/c` and `a/b-c` apart, but a repository name may contain `_` and punctuation sanitizes to dashes, so two repositories can still share a stem; `--out` names a path. Dot segments (`.` and `..`) in a URL are taken literally, never normalized.
...
- **Flags (`mail`):**
  ...
  - `--as <seat>`: Serve exactly this seat's mail. The seat must be one this repo resolves — the structural set (config / seat dirs / cohort map), and **`--as` alone also accepts the env-declared list when that structural set is empty**: the flag is a declaration, so it is served on a checkout that cannot corroborate it. (`--derive-seat` deliberately does not do this; see its bullet.) A foreign seat is refused, because its processed-marks cursor lives in its home repo and a foreign-anchored poll answers "ever addressed," never "unread." Poll-only: the flag is parent-scoped, so `mail reply` ignores it — use `mail reply --from <seat>`.
  - `--all-seats`: Serve the full multi-seat union (repo dashboard view) — bypasses the identity gate by name.
  - `--derive-seat`: Answer the prior question — which seat is this session? — and poll nothing (mmnto-ai/totem#2801; the signon step-0 probe). Prints exactly one stdout line, `seat=<id> source=env`, and exits `0` when the inherited `TOTEM_SELF_AGENT` names exactly one seat this repo hosts. **Hosts** means the structural set — in this flag's help, in this bullet and in the signon skill: `.totem/orchestration/config.json` `host_agents`, else the seat directories, else the cohort map keyed on the **origin repository** (the `origin` remote's repo name, else the repo-root basename, matched case-insensitively — so a per-agent worktree or a renamed clone of a cohort repo resolves its real seats). The env never contributes to that set, and this is where the flag differs from `--as` above: `--as` is a declaration and is served even where the structural set is empty, while `--derive-seat` corroborates a declaration, which a hosted set the env feeds cannot do. A refusal on a repo with no registration at all therefore means the repo cannot corroborate you, not that your seat is wrong. Otherwise a stderr refusal naming the supplied value (or `unset`) and every seat this repo hosts, exit `2`; a repo hosting no seat is refused with the `totem seat add` cure, and a repo that hosts seats none of which you declared is refused against that list. A single-seat repo with the env unset still refuses — a config- or dirs-derived resolution says what the repo hosts, never what this session is. Contradictory with `--as` and `--all-seats`. Opens no outbox and no `processed/` dir. Under `--json`: one stdout object in both arms — `{ ok: true, seat, source: 'env', line }` or `{ ok: false, refusal }` — with the same exit codes and nothing on stderr, the poll's own `--json` contract.
...
- **Exit codes:** `0` — verdict derived. `2` — NOT DERIVED (no self agent resolved, an identity-gated poll, or a flag contradiction — `--derive-seat` passed with `--as <seat>` or `--all-seats`, which declare the seat it asks for): fix identity or the flags and re-run. `4` — SENDER FAULT (mmnto-ai/totem#2685): the verdict IS derived and rendered, but an outbox this repo hosts for a resolved seat carries a dispatch whose `to:` matches no roster agent — undeliverable to every seat-scoped poll (`to:` is single-valued per ADR-098; a comma list is refused by `mail send` and faults when hand-written). Rendered as an `Error:` line and as a structured `senderFaults[]` entry under `--json`; fix the `to:` and re-poll. A foreign repo's undeliverable dispatch stays a `Warning:`. `2` wins over `4` when both hold. Like a gated poll's warnings, an `Error:` line names a dispatch basename — propagate nothing from it. `2` is also the REFUSED-ROOT arm (mmnto-ai/totem#2946, mmnto-ai/totem#2968): the poll, `mail mark` and `ecl-gc` derive their repository from the nearest `.git` toplevel (a stray `.totem/` under a subdirectory never captures them — mmnto-ai/totem#2938), and a start outside any repository, inside a linked worktree, in a repository with no `.totem/` (not a Totem repository, as `mail send` already refuses), or whose `.git` entry cannot be read is refused with one line naming the cure — run the verb from the resident checkout that hosts your seat, or `totem init` the repository — before anything is read or written.
- **Subcommands:**
  ...
  - `mail mark <source>` marks a consumed dispatch processed in your own `processed/` cursor (consume-without-reply, ADR-106 § A1.4). It runs from the resident checkout that hosts the seat: a linked worktree, a repository with no `.totem/`, or a directory outside any repository is refused (exit `2`) so no phantom `processed/` store is minted.
