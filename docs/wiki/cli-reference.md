...
- **The `AGENTS.md` floor:** when the repository has no `AGENTS.md`, init writes one — a title, a managed span between `<!-- totem:agents-floor:start -->` and `<!-- totem:agents-floor:end -->` carrying the vendor-neutral instruction set (session start, working rules, review-bot conduct, the installed skills), and a stub for your own conventions. Every later `totem init` refreshes exactly the span and leaves every other byte alone. An existing `AGENTS.md` without the two markers is yours and is never rewritten; add the marker lines where the managed span should sit and re-run `totem init` to adopt it. A marker counts only as a whole line, so quoting one in an inline code span, mid-sentence, in an indented code block or inside a closed fenced block is safe prose; and a start marker followed by an end marker is a span by definition, so never leave stray markers around your own text. `totem eject` removes every complete span (and the file, when the span was all it held), leaving one blank line at the seam and naming any stray marker it leaves behind.
