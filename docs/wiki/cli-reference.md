...
- **Flags (`mail`):**
  ...
  - `--derive-seat`: Answer the prior question — which seat is this session? — and poll nothing (mmnto-ai/totem#2801; the signon step-0 probe). Prints exactly one stdout line, `seat=<id> source=env`, and exits `0` when the inherited `TOTEM_SELF_AGENT` names exactly one seat this repo hosts. **Hosts** has one definition throughout: `.totem/orchestration/config.json` `host_agents`, else the seat directories, else the cohort map keyed on the **origin repository** (the `origin` remote's repo name, else the repo-root basename — so a per-agent worktree or a renamed clone of a cohort repo resolves its real seats). The env never contributes to that set: the probe's value is that it can refuse a supplied identity, which a hosted set the env feeds cannot do. Otherwise a stderr refusal naming the supplied value (or `unset`) and every seat this repo hosts, exit `2`; a repo hosting no seat is refused with the `totem seat add` cure, and a repo that hosts seats none of which you declared is refused against that list. A single-seat repo with the env unset still refuses — a config- or dirs-derived resolution says what the repo hosts, never what this session is. Contradictory with `--as` and `--all-seats`. Opens no outbox and no `processed/` dir.
