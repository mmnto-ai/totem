...
- **Flags (`mail`):**
  ...
  - `--derive-seat`: Answer the prior question — which seat is this session? — and poll nothing (mmnto-ai/totem#2801; the signon step-0 probe). Prints exactly one stdout line, `seat=<id> source=env`, and exits `0` when the inherited `TOTEM_SELF_AGENT` names exactly one seat this repo hosts — hosted-ness derived the way `--as` derives it, the env-declared list included when the structural union is empty. Otherwise a stderr refusal naming the supplied value (or `unset`) and every seat this repo hosts, exit `2`; a repo hosting no seat is refused with the `totem seat add` cure. It adopts no seat you did not declare: a config- or dirs-derived resolution says what the repo hosts, never what this session is, so a single-seat repo with the env unset still refuses. Contradictory with `--as` and `--all-seats`. Opens no outbox and no `processed/` dir.
