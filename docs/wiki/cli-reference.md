...
- **Stray Markers (sensor row, no gate):** every `.totem/` directory under the repository toplevel that is not its root marker, split by what git tracks — minted residue such as `packages/cli/.totem/temp` or a phantom `apps/<x>/.totem/orchestration/` warns with the remedy, a committed fixture's `.totem` is named and passes, and a nested repository's own marker is neither (mmnto-ai/totem#2938). Until that fix a reader verb run beneath one resolved it as the repo root; the toplevel now wins the walk, and the row names what is still minting them. A sweep its bounds stopped says so rather than claiming a clean tree.
