...
- **Enforcement tier on a rewrite:** with no `--strict` / `--standard` flag and no `hooks.tier` in config, each hook is re-rendered at the tier the installed hook itself declares — keeping a hook current never silently downgrades a strict one. Every hook write is atomic (a same-directory temp, then a rename), so an interrupted install leaves the old file or the new one, never a truncated hook.
