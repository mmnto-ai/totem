...
- **Enforcement tier on a rewrite:** with no `--strict` / `--standard` flag and no `hooks.tier` in config, each hook is re-rendered at the tier the installed hook itself declares — keeping a hook current never silently downgrades a strict one.
