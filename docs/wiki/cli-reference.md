...
Resolves the bot review threads a round has already dispositioned (mmnto-ai/totem#2841) — the lawful way past the merge-ready gate's predicate 2, which denies a merge while an unresolved, non-outdated bot-rooted thread is open. Zero LLM, **dry-run by default**: it prints one row per bot-rooted thread carrying that thread's REST root comment id and a verdict (`resolve`, `skip:already-resolved`, `skip:outdated`, `skip:no-evidence`, `skip:not-selected`), and mutates only under `--apply`. A thread is resolved ONLY with disposition evidence — an in-thread reply from a non-bot, or a non-bot PR-level comment created after the thread's root (the round-disposition comment, the only lawful answer to a GCA thread) — so a `skip:no-evidence` row is never resolved under any flag. `--ids <comma-separated REST root comment ids>` narrows the batch and an unmatched id aborts before anything is resolved; `--json` emits the same rows as one document. The verb never posts a comment, a reply or a review. Exit codes: `0` plan printed / applied clean, `1` a read that did not complete (nothing resolved) or `gh` absent, `2` an unmatched id, a failed mutation, or a selected thread skipped for want of evidence under `--apply`.
