...
- **Stray Markers (sensor row, never gates):** every `.totem/` directory under the repository that is not its root marker — `packages/cli/.totem/temp`, a committed fixture's `.totem`, a phantom `apps/<x>/.totem/orchestration/` — with the remedy (mmnto-ai/totem#2938). Until that fix a reader verb run beneath one resolved it as the repo root; the toplevel now wins the walk, and the row names what is still minting them.
...
- **Subcommands:**
  ...
  - `mail mark <source>` marks a consumed dispatch processed in your own `processed/` cursor (consume-without-reply, ADR-106 § A1.4). It runs from the resident checkout that hosts the seat: a linked worktree, or a directory outside any repository, is refused (exit `2`) so no phantom `processed/` store is ever minted.
