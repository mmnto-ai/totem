...
*Note: Running `pnpm install` will automatically trigger the `prepare` script, which installs git hooks.*
...
*Note: Always run `pnpm run format` before committing new files or modifications.*
