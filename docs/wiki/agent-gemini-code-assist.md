...
Totem influences GCA primarily through compiled architectural rules exported during `totem lesson compile --export` or `totem review`, which can inform the `.gemini/styleguide.md`. GCA does not use the `search_knowledge` MCP tool, as it cannot run local MCP servers.
