...
`totem init` installs Claude Code skills at `.claude/skills/<name>/SKILL.md`, and every later `totem init` refreshes them in place. Wherever the directory you run `totem init` in carries an `.agents/` directory, the same managed block is written to `.agents/skills/<name>/SKILL.md` as well, whichever AI tools were detected (that surface is read by Gemini CLI, Antigravity, Kimi and Codex, not by Claude Code; a `--bare` init skips every AI-tool surface, both copies included), and the install summary lists each twin; the two copies are equal inside the markers, and each keeps its own extension tail below the end marker. A repository without that directory gets one summary line saying the twins were not written, and `totem eject` removes the twins it wrote alongside the `.claude` copies (mmnto-ai/totem#2788, mmnto-ai/totem#2899). Each managed file is fenced by two marker comments, `<!-- totem:skill-start -->` and `<!-- totem:skill-end -->`. The refresh contract:
