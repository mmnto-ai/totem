...
- **All review modes count** — the standard path, `--mode structural`, and the multi-lane fan — each recorded once a verdict actually exists. A review that threw records nothing, matching `spec`, which records only after synthesis produced content. Structural mode is context-blind and builds no grounding bundle, so "review grounding" is loose for it: what is counted is the agent-initiated review action.
