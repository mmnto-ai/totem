...
- **All review modes count** — the standard path, `--mode structural`, and the multi-lane fan — each recorded once a verdict has actually **parsed**. A review that threw, one run with `--raw` (a context dump with no verdict at all), and one whose verdict never parsed all record nothing; a **FAIL** verdict does record, because a failing review is a completed derive and excluding it would shrink the denominator. This matches `spec`, which records only after synthesis produced content. Structural mode is context-blind and builds no grounding bundle, so "review grounding" is loose for it: what is counted is the agent-initiated review action.
