...
When you run `totem init`, Totem auto-detects your ecosystem (by looking for `requirements.txt`, `Cargo.toml`, or `go.mod`) and automatically merges the relevant language pack into `.totem/lessons/baseline.md` and `.totem/compiled-rules.json`.
