...
- [ ] **upstream-feedback/049 substrate response.** Two options not mutually exclusive: (A) local lint pack in totem-strategy for doubled-slug variants + bare-ref forms + section-anchor + truncation residues; (B) lift to `@mmnto/pack-governance` or built-in lint pack for cross-repo adoption. Recommended sequencing: (A) first as fast-feedback proof, then (B) if false-positive rate stays low.
