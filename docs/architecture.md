...
The compilation process reads files directly from disk instead of parsing staged diffs to prevent false positives. Developers can bypass false positives using audited inline suppression directives. Rules are scoped using anchored glob matching to prevent leaks outside specified directories. The system relies on a curated 439-rule set for baseline enforcement.
