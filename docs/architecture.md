...
- **Workflow & Evaluation:**
  - **Planning & Orchestration:**
    ...
    - _Hooks:_ Enforces lifecycle events like `/prepush` via `PreToolUse` hooks, integrating phase-gate enforcement to actively warn on commits lacking proper preflight. Pipeline reliability is bolstered by robust `PostCompact` formatting, now expanded with a comprehensive capability manifest.
  - **Review & Quality:**
