...
- **Rule Testing & Extraction:**
  - **Capture & Extraction:** Enables inline capture and batch PR lesson extraction. Lessons are strictly Zod-validated before disk writes to ensure structural integrity. A **Retirement Ledger** (`.totem/retired-lessons.json`) tracks intentionally removed lessons to prevent re-extraction during future import cycles.
